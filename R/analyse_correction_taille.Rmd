...
if (exists("userdistant") & exists("passworddistant") & exists("hostdistant")) {
	if (!exists("mainpass")) pois <- getPass(msg="main password")
	pool <- dbPool(
			drv = Postgres(),
...
file <- "T:\\12-PoleMAB\\Socle\\Migrateurs\\poissons\\suivi_pab\\2022\\Concat22_jusqua80958.txt"
...
# Données de taille sans indication de la zone
ssm[ssm$taillepixel>0 & is.na(ssm$co_zone), ]
...
ssmf <- ssm[,c("fichierVideo",
				...
				"taille",
				"remarque")]
file <- "C:/workspace/pdata/traitement_stacomi/rapport_passe/data/2021/fichiers_traite_2021.SSM"
