...
%%	 lancer avec Knitr + Xelatex + biber
...
\usepackage[UTF-8]{inputenc} %encodage du fichier source
...
du barrage d'Arzal, Rapport 2025)},
...
\fancyhead[LO,RE]{\slshape \normalsize Rapport passe à bassins 2025}
...
\Large \textbf{
    ...
    de la passe à bassins du barrage d'Arzal (Vilaine).
...
\qquad \color{vert_clair_BGM}\large{\titlefont Août 2026}
\end{minipage}
%
...
\bigskip
{\titlefont  \LARGE rapport 2025\par }
\vspace{4mm}\noindent
{\color{turquoise_EV}\rule{0.9\textwidth}{1.8pt}}\par
\vspace{5mm}
{\color{jaune_EV}{\titlefont \Large \itshape{Cédric Briand, Brice Sauvaget, Gérard Eriau\par   }}}
\end{center}
\end{minipage}
...
\begin{minipage}{0.8\textwidth}
\begin{center}
\hspace{3cm}{\color{white}{\huge \itshape{Août 2025}}}
\end{center}
\end{minipage}
\restoregeometry
\clearpage
...
L'estuaire en amont, jusqu'à la limite historique de l'estuaire tital (barrages
de la Potinais et de Malon, respectivement à 45 et 77 km en amont), a été
transformé en rivière.
Le plan d'eau ainsi créé a vu se développer de nouveaux usages dont les
principaux sont la navigation et la production d'eau potable.
Le barrage d'Arzal joue un rôle majeur dans la protection contre les crues
de l'aval du bassin versant, en empêchant les conjonctions de forts débits amont
et d'importantes surcotes de marées.
...
poissons migrateurs par la passe à bassins en 2025 
\footnote{La migration sur les passes à anguilles est traitée dans un autre
rapport :
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/IQByFprhrt2pTowh53k53SD6ASxL7ibVFXkpiAGLdeZ91LI?e=d4jCE3}{Gestion
de l'anguille sur le bassin versant de la Vilaine, 2024}.}
...
\ref{photo_barrage}).
Il bloque l’onde de marée à 10 kilomètres de l’embouchure et constitue une
rupture nette entre le milieu estuarien et le plan d’eau douce artificiellement créé à l’amont.
Le marnage à l’aval est de 6 mètres lors des grandes marées (Figure
\ref{photo_barrage}).
...
dispositif spécifique pour les anguilles composé de trois rampes de reptation
plongeant dans l'estuaire et permettant aux anguillettes et aux civelles de grimper le long des rampes en
...
Les paramètres hydrauliques de la passe à bassins conditionnent le passage des
espèces migratrices : le courant doit être suffisant pour attirer le poisson vers l’entrée de la passe, mais pas trop puissant pour lui permettre de franchir les chutes
...
(saumon, truite de mer, aloses, lamproie, mulet, anguille jaune) et d’autres
...
Une contrainte de sécurité de 30 cm sert à empêcher les intrusions d’eau salée
dans le plan d’eau amont.
La passe reste en fonctionnement à basse mer mais on considère qu’au-delà de 50
cm, la chute à l’aval de la passe n’est plus franchissable. Lorsque le niveau
dans le dernier bassin à l’aval de la passe (niveau sas) est situé 50 cm en
dessus du niveau mer, la passe est considérée en dysfonctionnement hydraulique.
...
fonctionnement du dispositif, 1 = fonctionnement normal, 2 = arrêt normal dû au
fonctionnement du dispositif, ici marée haute, 3 = arrêt pour maintenance, 4 =
dysfonctionnement hydraulique, 5 = pas de données. Le graphique est créé à
partir du package DiagrammeR
...
fonctionnement normal la passe aurait été fermée car le niveau de la mer
s'approche à moins de 30 cm du niveau de la Vilaine, ce qui aurait déclenché
la fermeture de la vanne amont pour éviter les intrusions d'eau salée (Figure
\ref{detail_fonct_passe_2} en annexe).
...
considérons que la passe reste attractive car l'exutoire des tuyaux du siphon
se situe près de l'entrée de la passe. De plus, il n'est pas nécessaire de
maintenir un débit d'attrait important pour que les poissons trouvent la source
principale de l'eau douce s'écoulant dans l'estuaire, et dont la salinité est
proche de l'eau de mer en absence de débit.
...
système de grille vers un couloir de 40 cm de large équipé de vitres. Un caisson
de rétroéclairage permet de filmer les poissons en ombres chinoises. Le système
d'enregistrement se compose d'une caméra et d'un ordinateur pour
l'enregistrement (Figure \ref{photo:suivi_numerique}). Le système de comptage
est basé sur un enregistrement numérique des passages de poissons réalisé à
l’aide des logiciels SYSIPAP (Système de Surveillance Informatisée des Passes
à Poissons).
...
les différentes espèces et rentrer les effectifs correspondant à l’aide de
l’interface "Choix espèce". Le résultat du dépouillement est un fichier texte
comprenant la liste des événements enregistrés : espèce, montée ou descente,
numéro de l’image de sortie, date et heure, taille en pixel et distance du poisson à la vitre.
...
a une grande incidence sur sa franchissabilité. Le débit du fleuve peut être
évacué par des vannes wagons ou par des clapets flottants (volets) en surverse.
...
Entre 2013 et 2023, nous avons observé grâce à un sonar multifaisceaux (Didson)
des passages de lamproies marines sous les vannes lorsque les vitesses de courant
ne sont pas élevées, les effectifs recalculés pouvant être conséquents.
Lorsque la marée monte jusqu’au niveau des volets, il arrive un moment où ils sont franchissables par des espèces
ayant une bonne capacité de nage. Les chiffres donnés pour l’évaluation des
stocks de migrateurs doivent donc être considérés comme un minimum.

\section{Résultats}
\subsection{Fonctionnement de la passe à bassins}
\paragraph{Résumé du fonctionnement.}

Le fonctionnement de la passe en \Sexpr{CY} a été perturbé par la grande crue de
janvier - février et par les travaux sur les gabions et les siphons.
La passe a été relancée le 18 février, à ce stade, les travaux des
gabions ne sont pas terminés, et les support des siphons ont été posés mais pas les coudes
finaux. Le 19 février, la passe est arrêtée en journée pour intervenir sur les
siphons, elle sera ensuite arrêtée du 26/02 au 06/03 pour cause de crue. En
avril et mai, des problèmes techniques conduisent au changement des fins de
course de la vanne amont. En mai et juin, la passe est manoeuvrée en fermeture à
basse mer pour capturer des lamproies marines dans le cadre de l'étude INRAE
HALAMEAUP. Du 24/12 au 03/03/26, une panne sur le distributeur hydraulique de la
vanne intermédiaire a entraîné l'arrêt de la passe (Figure \ref{report_df_annee} en annexe).

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/fonctionnement_passe.png}
...
La passe a été batardée le 4 juillet, mise en mode étiage du 05/08 au 04/09,
puis débatardée le 21 octobre (Figure \ref{debit_passe_comp}).

En Figure \ref{report_df_fct_2003_2025}, l'analyse des durées mensuelles
...
graphiques sont présentés pour la période 2002-\Sexpr{CY} (Figure
\ref{report_df_2003_2025}) en mettant juste en avant les périodes où la passe est fonctionnelle
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/debit_passe_comp.png}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/report_df_fct_2003_2025.png}
\caption{Fonctionnement de la passe de 2002 à 2025, avec les détails par type
de fonctionnement.}
\label{report_df_fct_2003_2025}
\end{figure}%
%
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/report_df_2003_2025.png}
\caption{Fonctionnement de la passe de 2002 à 2025.}
\label{report_df_2003_2025}
\end{figure}%
%

\paragraph{Fonctionnement du dispositif de comptage.}

Le dispositif de comptage a été mis en service le 18 février et arrêté le 17
décembre, sans interruption de fonctionnement (Figure \ref{report_dc_2025}).

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/report_dc_2025.png}
\caption{Bilan du fonctionnement du dispositif de comptage en \Sexpr{CY}.}
\label{report_dc_2025}
\end{figure}

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/report_dc_1998_2025.png}
\caption{Bilan du fonctionnement du dispositif de comptage de 1998 à 2025.}
\label{report_dc_1998_2025}
\end{figure}
...
calc_fonct_pass <- function(dat, per_dis_identifiant=1, vvv){
  ...
  dat[dat$hauteur_vanne_vilaine==-0.2 & is.na(dat$per_etat_fonctionnement) & !is.na(dat$hauteur_vanne_vilaine),
      "per_tar_code"] <- 3
  dat[dat$hauteur_vanne_vilaine==-0.2 & is.na(dat$per_etat_fonctionnement) & !is.na(dat$hauteur_vanne_vilaine),
      "per_etat_fonctionnement"] <- FALSE

  dat[dat$hauteur_vanne_mer==1.72 & is.na(dat$per_etat_fonctionnement)& !is.na(dat$hauteur_vanne_mer),
      "per_tar_code"] <- 3
  dat[dat$hauteur_vanne_mer==1.72 & is.na(dat$per_etat_fonctionnement)& !is.na(dat$hauteur_vanne_mer),
      "per_etat_fonctionnement"] <- FALSE
...
# from the server works with no password

con <-dbConnect(RMySQL::MySQL(),
dbname = "archive_IAV",
username = decrypt_string(umysql,pois),
    #password = decrypt_string(pwdmysql,pois),
	host = decrypt_string(hostmysql,pois),
	 port = 3306)
df<- tribble(
    ~name, ~table, ~tag,
    "niveau_mer_passe","b_passeapoisson_niveaumer",   0,
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# pb heures arzal (ce n'est pas bon, seulement après 20 --------
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

# The dam was in GMT+1 before 2016
# When converting from GMT we need to correct one hour delay between GMT
# and the true value which is at GMT + 1. To get to GMT we need to remove
# one hour


# this just reads the time as CEST
#strptime("2025-04-08 08:00:00",tz = "Europe/Paris",format="%Y-%m-%d %H:%M:%S")
# this one will shift 2 hours
#lubridate::with_tz(
#  lubridate::ymd_hms("2025-04-08 08:00:00",tz="GMT"),
#   tzone = "Europe/Paris")
# before 2016
if (CY <=2015) {
res1 <- res1 |>
    mutate(time = as.POSIXct(strptime(HoroDate,tz="GMT",format="%Y-%m-%d %H:%M:%S")) -
        ...
        time=lubridate::with_tz(time, tzone = "Europe/Paris"))
 # this convert to the new time (the absolute time is still the same)
    # use force_tz otherwise when the tz is wrong
} else {
  res1 <- res1 |>
      mutate(time = as.POSIXct(strptime(HoroDate,tz = "Europe/Paris",format="%Y-%m-%d %H:%M:%S")))
}
tz(res1$time)
...
graph_DF_detail(data_DF, from="18/04/2025", to="25-04-2025")
dev.off()

png(filename=file.path(imgwd,CY,"detail_fonct_passe_2.png"),7,7,units="in",res=300)
graph_DF_detail(data_DF=data_DF, from="01-03-2025", to="19-03-2025")
dev.off()


graph_DF_detail(data_DF, from="08/04/2024", to="09/04/2024")
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
dbExecute(req@connection,glue("DELETE FROM iav.t_periodefonctdispositif_per
            WHERE per_dis_identifiant=1
            AND extract(year from per_date_debut)={CY}") )
...
DBI::dbExecute(req@connection, "INSERT INTO iav.t_periodefonctdispositif_per (
        ...
        , per_tar_code
        ,'IAV' as per_org_code
        FROM t_periodefonctdispositif_per_temp") #2462
...
# mais ici j'ai fait des modifications manuelles en sql/traitementç2025
...
print(g)
dev.off()
#Sys.setenv(TZ="Europe/London") # GMT
...
png(filename=paste0(imgwdy,"report_df_2003_2025.png"),width=6,height=12, units="in",res=300)
...
png(filename=paste0(imgwdy,"report_df_fct_2003_2025.png"),width=6,height=12, units="in",res=300)
...
#png(filename=paste0(imgwdy,"report_df_2011_2025.png"),width=8,height=12, units="in",res=300)
...
#dev.off()
#
#png(filename=paste0(imgwdy,"report_df_fct_2011_2025.png"),width=8,height=12, units="in",res=300)
...
png(filename=paste0(imgwdy,"report_dc_1998_2025.png"),width=6,height=12, units="in",res=300)
g_report_dc_1 +
...
#    date_debut_mesure = "2025-01-01",
#    date_fin_mesure="2025-12-31",
...
req<- new("RequeteDB")
req <- connect(req)
res <- dbGetQuery(req@connection,glue("SELECT distinct stm_libelle FROM iav.tj_stationmesure_stm
...
dbExecute(req@connection,glue("DELETE FROM  iav.tj_conditionenvironnementale_env
            WHERE env_stm_identifiant in (6, 21, 22, 19, 24, 25)
            AND extract(year from env_date_debut)={CY}") ) # 0
...
DBI::dbExecute(req@connection, "INSERT INTO iav.tj_conditionenvironnementale_env (
        ...
        ,env_stm_identifiant
        ,env_org_code
        FROM tj_conditionenvironnementale_env_temp") #2012 (2021) 2309 (2022) 1982 (2024) 2382 (2025)
...
L'année \Sexpr{CY} a été marquée par des débits de crue très importants au
premier trimestre, et forts en décembre (Figure \ref{fig_debit}).
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/fig_debit.png}
\caption{Débits de la Vilaine en \Sexpr{CY}. En bleu = débits mesurés au
Pont de Cran, à l'aide d'une sonde à ultrasons, en orange = débits
instantanés au barrage d'Arzal (volets et vannes). Chaque point représente la valeur moyenne calculée toute les
...
<<correction_taille_tous, echo=FALSE, eval=FALSE, include=FALSE >>=
# pb de tailles des poissons aux vitres
options(warn=-1)
...
r_sample_char<-choice_c(r_sample_char,
    dc=c(5),
    taxa=c(2183,2014,2234,2055,2038,2220,2224), # 2238 Silure
    stage=c(11,'AGJ',"IND"),
    par=c("C001"),
    horodatedebut="2007-01-01",
    horodatefin=str_c(CY,"-12-31"),
    silent=TRUE)
# lancement
...
# récupération des distances au sein des commentaires de lots


#req<- new("RequeteDB")
#req <- connect(req)
#res <- dbGetQuery(req@connection,glue("SELECT lot_identifiant, lot_commentaires
#            FROM iav.t_operation_ope JOIN iav.t_lot_lot on lot_ope_identifiant=ope_identifiant
#            WHERE (ope_date_debut, ope_date_fin) overlaps ('2007-01-01'::timestamp without time zone, '2025-12-31 23:59:59'::timestamp without time zone)
#            AND ope_dic_identifiant in('5')
#            AND lot_tax_code in ('2038','2055','2234','2183','2014','2220','2224')
#            AND lot_std_code in ('11','AGJ','IND')
#            ;"))
...
tail <- r_sample_char@data
xtabs(tail$lot_effectif  ~tail$tax_nom_latin)
#tail$tax_stade <- str_c(tail$tax_nom_latin, tail$std_libelle)
# graphique des données quantitatives
png(filename=paste0(imgwdy,"taille_tous.png"),width=600,height=400)

g <- ggplot(tail)
g <- g+
    geom_jitter(aes(x=ope_date_debut,y=car_valeur_quantitatif, col=lot_tax_code),
        width = 0.5,
        height = 0.5,
        alpha=0.5) +
    scale_colour_discrete_qualitative(
        palette = "Pastel1", nmax = 20, order = 1:20)+
    xlab("Date")+
    ylab("Taille tous poissons (mm)")+
    theme_dark()
print(g)
dev.off()
options(warn=0)

unique(tail$car_par_code)
unique(tail$car_methode_obtention)

tail <- tail |>
    select (date= ope_date_debut, stade=lot_std_code,tax = tax_nom_latin,std_libelle,
        taille= car_valeur_quantitatif, annee, mois, quinzaine, semaine,jour_365) |>
    mutate(annee=as.factor(annee)
    )
tail <- tail[!(tail$tax=="Dicentrarchus labrax"&(tail$taille>1000 |tail$taille<200)),]
tail <- tail[!(tail$tax=="Anguilla anguilla"&(tail$taille>1000)),]
tail$anneenum <- as.numeric(as.character(tail$annee))
tail$moisnum <- as.numeric(as.character(tail$mois))
tail$tax<-as.factor(tail$tax)
g0 <- glm(taille ~ annee, data= tail)
pdev <- function(mod)(mod$null.deviance-deviance(mod))/mod$null.deviance
x11()
pdev(g0) # 0.07
plot(g0)
g1 <- glm(taille ~  annee*tax + mois*tax, data= tail )
g1 <- glm(taille ~  annee + tax + annee:tax, data= tail)
pdev(g1) #0.65
plot(g1)


library(mgcv)
g2 <- gam(taille ~ annee+ tax  + s(moisnum, k=7, by=tax, bs="cc"), data= tail)
plot.gam(g2, all.terms =T)
pdev(g2) # 0.691
g3 <- glm(taille ~ annee+ tax + annee:tax + as.factor(mois)*tax, data=tail)
pdev(g3) # 0.706
g4 <- glm(taille ~ annee+ tax + as.factor(mois)*tax, data=tail)
pdev(g4) # 0.692
plot(g4)
g5 <- gam(taille ~ annee+ tax  + s(anneenum, k=7, by=tax, bs="tp")+ s(moisnum, k=7, by=tax, bs="cc"), data= tail)
summary(g5) #70.1
plot.gam(g5, all.terms =T,ylim =c(-100,100),residuals=TRUE)
# trois anguilles
tail["19786",]
tail["13647",]
tail["13226",]
# les trois suivants : OK
tail["7935",]
tail["37859",]
tail["13737",]

co <- coefficients(g5)
co_annee <- co[grepl("annee", names(co)) & !grepl("anneenum", names(co))]
names(co_annee) <- gsub("annee","",names(co_annee))
barplot(co_annee)

req<- new("RequeteDB")
req <- connect(req)
res <- dbGetQuery(req@connection,glue("SELECT count(*) as nline, extract(year FROM ope_date_debut) as year
            FROM iav.t_operation_ope
            JOIN iav.t_lot_lot ON lot_ope_identifiant= ope_identifiant
            WHERE lot_commentaires SIMILAR TO '%mm[0-9]*mm%'
            GROUP BY extract(year from ope_date_debut)
            ORDER BY extract(year from ope_date_debut)"))
...
\citep{briand_stacomir_2017}, l'espèce est néanmoins classée en
préoccupation mineure sur la liste rouge de l'IUCN.
Sur la Vilaine au barrage d’Arzal, parmi les mulets, seul \textit{Chelon
ramada} remonte l’estuaire pour migrer en eau douce \citep{briand_suivi_2018}.
En effet, le passage des poissons entre le milieu estuarien et la rivière
provoque une variation brutale de salinité que seule \textit{Chelon ramada} pourrait
supporter à l’aller et au retour de migration \citep{briand_suivi_2018}.
La présence de la seule espèce euryhaline, et le confinement à la zone
estuarienne des autres espèces communes en Bretagne \textit{Chelon labrosus} et
\textit{Chelon aurata}, est cependant à confirmer
\citep{gautier_mulets_2005,trancart_analyse_2011}.}
\end{minipage}}
\smallskip
...
1997, en 2005, et depuis 2013. Lors des pics de migration, le comptage
des mulets est effectué par lots de 10 individus en cas de forts passages,
et la taille des poissons est analysée sur des échantillons hebdomadaires.
La structure en taille des mulets présente à la fois des variations saisonnières
et interannuelles (Figure \ref{figure_taille_mulets}). Les mulets les plus grands
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/graphe_taille_mulets.pdf}
\caption{Tendance saisonnière d'évolution des
tailles de mulets depuis 2013. A. Graphiques de densités (distributions) des
tailles par mois, en violet = printemps, en jaune = mois estivaux, en
rose = automne. B. Graphique en violon et box-plot des tailles annuelles. C.
Modèle gam ajusté sur les valeurs de taille (au centre modèle et intervalle de confiance
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/bilan_annuel_mup}
\caption{Bilan annuel des migrations de mulets au barrage d'Arzal.}
\label{bilan_annuel_mup}
\end{figure}

Les effectifs de mulets de 2025
(N=\Sexpr{format(vvv[["MUP"]][["NCY"]], digits=1, nsmall =0, big.mark="",scientific=FALSE)})
sont faibles comparés à ceux de la période 2014-2025 avec des variations entre
...
passe (Figure \ref{rmi_journalier_mup}) et des bancs commencent à
...
l'interprétation de la taille de la population de mulets en Vilaine, et un
calcul du nombre de mulets migrant jusqu'au mois d'août (inclu) est donné au
...
La migration des mulets est essentiellement diurne et matinale (Figure
\ref{horaires_mulet}), et elle dépend également du niveau mer avec une grande
part des passages pour des niveaux mer >0m (Figure \ref{horaires_maree_mulet}).

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/horaires_mulet}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/bmm_horaires_maree_mup}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.9\linewidth,keepaspectratio]{2025/rmi_journalier_mup.png}
...
# graphique des données quantitatives
png(filename=paste0(imgwdy,"taille.png"),width=600,height=400)
...
dev.off()
options(warn=0)
save(r_sample_char, file=str_c(datawdy,"r_sample_char_mulet.Rdata"))
@
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load(file=str_c(datawdy,"r_sample_char_mulet.Rdata"))
...
# g5mm <- gam(taille_mm ~ s(yday,bs="cc", by=year,k=8) + s(date,bs="re"),  data=dat) # random

# model 3 is best g4mm
anova(g3mm,g3mm,g4mm, test="Chisq")
...
pdf(file.path(imgwd,CY,"graphe_taille_mulets.pdf"),width=10, height=8)
print(p3)
dev.off()

#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# prédicton des  données avec loi normale +- 2 *sd autour de la moyenne ----
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# creation du tableau de prédiction
pred <-
    data.frame("date"=
            as.POSIXct(seq(from=as.POSIXct(dmy("01012013")), to = as.POSIXct(dmy("30072025")), by="1 day")))
pred <- fndate(pred)
...
save(bilA, file= file.path(datawd,CY,str_c("bilAmup",CY,".Rdata")))
...
load(file= file.path(datawd,CY,str_c("bilAmup",CY,".Rdata")))
res <- res[,c(1,2)]
restt<-bilA@data
...
#test graphique Bilan_annuel_mup : effectif de l'année + période de migration
res2 <-inner_join(restt,res)[!res$annee%in%c(2007,2008),c("annee","effectif","effectif_mont")]
data_long <- tidyr::gather(res2, key = "type", value = "value", -annee)
png(filename=str_c(imgwdy,"bilan_annuel_mup2.png"))
ggplot(na.omit(data_long), aes(x = factor(annee), y = value, fill = type)) +
		geom_bar(stat = "identity", position = "dodge") +
		labs(	x = "Année",
				y = "Effectif (mulet)",
				fill = "Type d'effectif") +
		scale_fill_manual(values = c(bleu_EV, orange_EV), labels = c("Effectif sur l'année", "Effectif sur la période")) +
		theme_minimal()
dev.off()
...
# ATTENTION CETTE FONCTION CHANGE LE TZ EN CEST ET C'EST TRES IMPORTANT
...
fun_date_extraction=function(data, # tableau de donnees e importer
  ...
  warning(sprintf('%s converted to tz "Europe/Paris"', nom_coldt))
  data[,{{nom_coldt}}] <- lubridate::force_tz(data[,{{nom_coldt}}], "Europe/Paris")
...
tz(bMM_Arzal@data$ope_date_debut) # UTC
...
mig <-	fun_date_extraction(
    ...
    jour_mois = TRUE,
    heure = TRUE)
tz(mig$ope_date_debut)
hhh <- mig |> group_by(heure) |> dplyr::summarize(N=sum(value))
...
print(g)
dev.off()
# hhh <- mig |> group_by(heure, mois) |> dplyr::summarize(N = sum(value))
load(file=file.path(datawd, CY, "fonct_pass.Rdata"))	# ll
...
ll$dat$time <- with_tz(ll$dat$time, "Europe/Paris")
#      ope_identifiant lot_identifiant      ope_date_debut        ope_date_fin
#         2407          308885          483074 2024-09-19 06:40:00 2024-09-19 06:50:00
mige <- inner_join(mig, ll$dat, by="time")

hhh<- mige |> group_by(heure) |> dplyr::summarize(N=sum(value))

png(file=str_c(imgwdy,"bmm_horaires_maree_mup.png"),7,7,units="in",res=300)
mige |> mutate(heure = as.numeric(heure)) |>
mutate(value = abs(value)) |>
ggplot()+
    geom_point( aes(x=heure,y=niveau_mer_barrage,col=value))+
    scale_colour_gradientn(colors=wesanderson::wes_palette("Zissou1", n = 5, type="continuous"), trans="log1p", breaks=c(0,1,10,100,1000,3000)) +
    scale_fill_gradientn(colors=wesanderson::wes_palette("Zissou1", n = 5, type="continuous"), trans="log1p", breaks=c(0,1,10,100,1000,3000)) +
    scale_x_continuous(breaks=c(6,12,18), minor_breaks=seq(0:23)) +
    xlab("Heure")+
    theme_bw()
dev.off()


png(file=str_c(imgwdy,"bmm_horaires_mareebismup.png"),7,7,units="in",res=300)
ggplot(mige)+
    geom_point( aes(x=timec,y=niveau_mer_barrage,col=value))+
    facet_wrap(~ per_tar_code) +
...
<<report_mig_interannual_mulet, echo=FALSE, eval=FALSE,include=FALSE >>=
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
bmi <- calcule(bmi)
#save(bmi,file=str_c(datawd,'bmimup2025.Rdata'))
#load(file=str_c(datawd,'bmi2025.Rdata'))
#str(bmi)

plot(bmi, year_choice = CY, silent=TRUE)
g <- get("g_standard",envir_stacomi)
...
g <- g+scale_x_datetime(name="date",
    ...
    ylab("Effectifs en migration dans la passe")+
    theme_bw()+
    theme(legend.key = element_blank())
print(g)
dev.off()

plot(bmi,plot.type="barchart", timesplit="mois", year_choice = CY, silent= TRUE)
...
plot(bmi,plot.type="density",timesplit="semaine", year_choice=CY, silent =TRUE)
...
color_ope <- c(rev(mycolorramp1(10)),mycolorramp2(nb_annee-11),"#85A3BA")
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
png(filename=str_c(imgwdy,"rmi_density_mup.png"))
g <- get("g_density",envir=envir_stacomi)
g <- g +
...
plot(bmi, plot.type="seasonal", timesplit="semaine", year_choice =CY, silent= TRUE)
dev.off()
...
mmm <- mmm[,-1]
options(encoding = "UTF-8")
colnames(mmm) <- c("Mois", "Min*", "Moyenne*", "Max*", "N (2025)")
xtmmm <- xtable(mmm,caption="Migrations mensuelles des mulets dans la passe à bassins du barrage d'Arzal. $N$ = nombre, * correspondent à
        la période 1996-2025. min* = minimum mensuel, max* = maximum mensuel, moyenne* = moyenne des effectifs mensuels.",
...
Après métamorphose, les smolts de lamproies dévalent en mer où elles vivent 1 à
2 ans en tant que parasite externe sur des poissons ou des mammifères.
...
ont été très mauvaises, puis les saisons 2020 et 2023 marquaient un retour à la
normale avant une nouvelle chute des effectifs cette année.
...
après 2015 ont réaugmenté à un niveau proche du niveau historique en 2020
(N=\Sexpr{num(vvv$LPM$N[match(2020,vvv$LPM$N$annee),1])}).
Cette année, les effectifs ont été de nouveau faibles avec
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/barplot_report_annual_lpm.png}
...
période 1996-\Sexpr{CY-1}, à l'exception du mois d'avril. Elles sont proches de
la moyenne en mars, très faibles et mai, puis quasi inexistantes ensuite
(Figure \ref{barchart_bmi_lpm}, Tableau \ref{table_lpm_mois}).

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/barchart_bmi_lpm.png}
...
en avril. Les premières migrations sont plus précoces que ce que l'on
pensait, et le suivi était souvent lancé trop tardivement avant 2021
...
lamproies marines en France par \citet{legrand_contrasting_2020} qui
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/rmi_density_lpm.png}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.49\textwidth]{2025/plot_seasonal_lpm.png}
\caption{Saisonnalité de la migration des lamproies marines, effectifs
hebdomadaires. Les barres représentent le 5ème et le 95ème percentile de la
migration, le point la médiane. L'importance des effectifs est représentée
par la couleur.}
\label{plot_seasonal_lpm}
\end{figure}

La comparaison des structures en taille vidéo laisse paraître un accroissement
de taille depuis 2017. C’est à partir de 2019 qu’une nouvelle méthode de mesure
a été appliquée : la taille a été mesurée relativement à 9 sections de la
vitre de comptage pour prendre en compte les déformations de la lentille de la caméra.
Or, l’augmentation est observée depuis 2017, et ne peut donc pas être attribuée
au changement de méthode de mesure. L’augmentation de la taille des lamproies
est donc un phénomène notable, comme pour les variations d’effectifs, les raisons
de cette augmentation de taille ne sont pas connues, et un phénomène exactement
similaire avait été rapportée sur la Loire (Marion Legrand, comm. pers.).


Après un pic de taille en 2020 et 2021, la taille baisse en 2022
et 2023, remonte en 2024, puis baisse à nouveau en 2025 tout en restant
au niveau de 2017 (Figure \ref{size_lpm}).


\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/size_lpm.png}
\caption{Comparaison des structure en taille des lamproies marines. Depuis 2019,
les tailles ont été calculées en prenant en compte la déformation des distances
dans les différentes zones de la vitre.}
\label{size_lpm}
\end{figure}
...
plot(rmi,plot.type="density",silent=TRUE, year_choice =CY)
g <- get("g_density",envir=envir_stacomi)
...
dev.off()
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
plot(rmi,plot.type="barchart",timesplit="mois", year_choice = CY, silent=TRUE)
g<-get("g_barchart",envir=envir_stacomi)
cols <- c("max" = "#012746","min" = "#6C3E00",">=moy" = "#327AB5", "<moy" = "#D29035","hist_mean"="black","hist_range"="grey","?"="#D26435")
fills <- c("max" = "#043C6B","min" = "#6C3E00",">=moy" = "#558AB5", "<moy" = "#D2A25F","hist_mean"="black","hist_range"="grey","?"="#D26435")
g <- g+scale_colour_manual(name=CY,values=cols,limits=c("min","max","<moy",">=moy","hist_mean","hist_range","?"))
...
plot(rmi, plot.type="seasonal", timesplit="semaine", year_choice = CY, silent = TRUE)
dev.off()
...
<<analyse_lpm, echo=FALSE, eval=FALSE, include=FALSE >>=
load(file=str_c(datawdy,"vvv.Rdata"))
total <- vvv[["LPM"]][["total"]]
fra <- readxl::read_excel(path=str_c(datawd,"comptage_frayere.xlsx")) |> rename("site"="...1")
fras <- fra |> rowwise() |>
    mutate(total = sum(c_across(where(is.numeric)),na.rm=TRUE)) |> select(site, total)
frat <- fra |>  summarise(across(where(is.numeric),  sum, na.rm=TRUE)) |>
    tidyr::pivot_longer(cols=everything(), names_to = "annee", values_to = "observee", names_transform =as.numeric)
fram <- fra |> tidyr::pivot_longer(cols=2:ncol(.),
        names_to = "annee",
        values_to = "observee",
        names_transform =as.numeric) |>
    mutate(observee=2*observee)  |>
    filter(annee !=2007)|>
    mutate(annee =as.factor(annee),
        )

# creation du graphique observe / corrige
# prediction à l'aide d'un modèle binomial
# graphique des sorties
png(filename=str_c(imgwdy, "frayere.png"))
fraf <- fram |> tidyr::complete() |>
    mutate(corrigee=predict(gnb<<- MASS::glm.nb(observee~site+annee, data=fram), newdata=., type="response")) |>
    group_by(annee) |> summarize(observee=sum(observee, na.rm=TRUE), corrigee= sum(corrigee, na.rm=TRUE)) |>
    mutate(annee=as.numeric(as.character(annee))) |>
    ungroup() |>
    right_join(frat|>select(annee), by="annee")|>
    tidyr::pivot_longer(cols=2:ncol(.),
        names_to = "type",
        values_to = "nombre") %T>%
    {print(ggplot(.)+
              geom_col(aes(x=annee,y=nombre,fill=type), position="dodge")+
              ylab("Nombre de frayères en Vilaine (source ONEMA, FDPPMA35)") +
              scale_fill_manual(values=c("observee"="#33857e", "corrigee"="#f2ae68")) +
              geom_vline(xintercept = 2007, linetype = 2, color = "black") +
              ggtext::geom_richtext(x = 2007, y = 100, label = "? Turbidité", angle = 90) +
              theme_bw()
      )
    }
#summary(gnb)
...
png(filename=str_c(imgwdy, "proportion_frayere.png"),4,4,units="in",res=300)
prop <- total |> select(effectif, annee) |>
    left_join(fraf|> tidyr::pivot_wider(id_cols="annee",names_from="type",values_from="nombre")) |>
    mutate(prop=round(effectif/corrigee,2)) |>
    mutate(prop =ifelse(prop>10,NA ,prop),
        prop=ifelse(prop<0.001, NA,prop)) %T>%
    {print(gsub1 <<- ggplot(.)+
              geom_point(aes(x=annee, y= prop)) +
    geom_smooth(aes(x=annee, y= prop), col="#505940", fill="#d8f1ae")+
    xlim(1996,2010)+
    theme_minimal())}
dev.off()

png(filename=str_c(imgwdy, "comp_frayere_arzal.png"), 7,7,units="in",res=300)
prop|> dplyr::select(annee, effectif, corrigee) |>
    rename("comptage Arzal"=effectif) |>
    tidyr::pivot_longer(cols=2:ncol(.),
        names_to = "type",
        values_to = "nombre") %T>%
   {print(ggplot(.)+
             geom_col(aes(x=annee,y=nombre,fill=type), position="dodge")+
             ylab("Effectif lamproie marines (source Eaux & Vilaine, ONEMA, FDPPMA35)") +
             ggrepel::geom_label_repel(aes(x=annee,y=effectif, label=prop), data=prop,box.padding = 0.5)+
             scale_fill_manual(values=c("corrigee"="#33857e", "comptage Arzal"=orange_EV)) +
             theme_bw())}
dev.off()
...
Clupéidés (sardine, hareng …).
La grande alose \textit{Alosa alosa} ne se reproduit généralement qu'une seule
fois en eau douce, en mai-juin, souvent sur sa rivière d'origine ou dans un
cours d'eau proche géographiquement. Les alosons restent à proximité des frayères quelques mois
...
En Bretagne, la situation était plutôt à l'amélioration, et la Vilaine faisait
partie des principaux sites avec le Blavet et l'Aulne. Après la période 2012-2016
où la pêche professionnelle estuarienne a fortement impacté la migration, on a
constaté un renforcement des effectifs sans toutefois jamais atteindre les
valeurs historiques des années 2000. Les effectifs sont de nouveau en baisse
marquée sur la Vilaine. L'espèce est classée en danger critique d’extinction en
France sur la liste rouge UICN.
}
\end{minipage}}
\smallskip
...
\Sexpr{CY} (Figure
\ref{rmi_journalier_ala} et \ref{tx_exploit}, Tableau
\ref{table_bilanannuel_ala}).
Cette migration place l'année \Sexpr{CY} au rang \Sexpr{vvv$ALA$rank}
par rapport à l'ensemble des années de migration, avec une augmentation marquée
par rapport aux années 2021-2024. En vidéo comptage, il n'est pas possible de
distinguer les deux espèces, grande alose \textit{Alosa alosa} et alose feinte \textit{Alosa fallax}.


%\begin{figure}[htpb]
%\centering
%\includegraphics[width=0.45\textwidth]{2025/bilan_annuel_ala.png}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/rmi_journalier_ala.png}
...
Globalement, les migrations sont inférieures à la moyenne interannuelle, à
l'excpetion de la première quinzaine d'avril (Figures \ref{rmi_journalier_ala}
et \ref{barchart_bmi_ala}, Tableau \ref{table_ala_mois}).

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/barchart_bmi_ala.png}
\caption{Comparaison de la migration d'aloses de \Sexpr{CY} avec la série
interannuelle de données 1996-\Sexpr{CY-1}.}
...
une augmentation de la taille moyenne après 3 années avec des petits individus.
...
Ainsi les tailles avant 2019 sont peut être légèrement plus petites (Figure \ref{size_alose}).
...
puis atteint des niveaux assez bas de 2022 à 2024. Ces variations de tailles
moyennes pourraient s'expliquer par des années pour lesquelles le succès reproducteur
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/size_alose.png}
...
Les premières aloses ont été observées assez tôt, à la mi-mars, et
la moitié de la migration était déjà réalisée début avril, ce qui en fait
l'année la plus précoce avec 2014 (Figure \ref{plot_seasonal_ala}).


\begin{figure}[htpb]
\centering
\includegraphics[width=0.49\textwidth]{2025/plot_seasonal_ala.png}
\caption{Saisonnalité de la migration des aloses, effectifs hebdomadaires. Les
barres représentent le 5ème et le 95ème percentile de la migration, le point
...
de Vilaine a fait l'objet d'un débat technique dans le cadre du Comité de
gestion des poissons migrateurs de Bretagne.
...
zones de reproduction a également été mis en avant (Figure \ref{tx_exploit}).

La question de la prédation du silure, dont les effectifs sont en claire
augmentation avec des tailles allant en s'accroissant, doit se poser, avec celle
de la régulation de cette espèce.


\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/bmm_horaires_maree_ala}
\caption{Passage des aloses en \Sexpr{CY}, par pas de temps de 10 minutes en
fonction de l'heure et du niveau mer.}
\label{horaires_maree_ala}
\end{figure}
...
\begin{figure}[pbt]
\centering
\includegraphics[width=0.5\textwidth]{2025/tx_exploit.png}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.9\linewidth]{2025/preco_diagram.png}
\caption{Diagramme de précaution pour la gestion des aloses en Vilaine. Les
données indiquent la position du stock par rapport à la règle de gestion proposée
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Horaires de passage des aloses
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mig <- bMM_Arzal@data |>
mutate(time = strftime(ope_date_debut, format="%H:%M:%S"),
		date=as.Date(ope_date_debut))
...
png(file=str_c(imgwdy,"bmm_horaires_maree_ala.png"),7,7,units="in",res=300)
ggplot(mige)+
		geom_jitter( aes(x=timec,y=niveau_mer_barrage,col=value))+
		scale_colour_gradientn(colors=wesanderson::wes_palette("Zissou1", n = 5, type="continuous")) +
		scale_fill_gradientn(colors=wesanderson::wes_palette("Zissou1", n = 5, type="continuous")) +
		scale_x_datetime(date_labels = "%H")+
		xlab("Heure")+
...
g <- g +	geom_violin(color=marron,
    ...
    theme_bw()+
    scale_color_manual(values=wesanderson::wes_palette("Zissou1", 8, type = "continuous"))+
    ylab("taille")
print(g)
dev.off()
...
bmi<-charge(bmi,silent=TRUE)
bmi<-connect(bmi,silent=FALSE, check=TRUE)
bmi<-calcule(bmi)
save(bmi,file=str_c(datawd,'bmiala2025.Rdata'))
#load(file=str_c(datawd,'bmi2025.Rdata'))
#str(bmi)
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
g <- g+scale_x_datetime(name="date",
        ...
        date_labels="%d-%m",
        limits=as.POSIXct(strptime(c("2000-03-15","2000-07-01"),format=,"%Y-%m-%d")))+
    ylab(iconv("Effectifs en migration dans la passe","UTF8"))+
    theme_bw()+
    theme(legend.key = element_blank())
print(g)
dev.off()
...
colrp1<-colorRampPalette(colors=c("#6D9A58","#C6B130"))
load_package("viridis")
len <- length(unique(bmi@data$bjo_annee))
rp1 <- round(len/2)
rp2 <- ifelse (is.odd(len), rp1+1,rp1)
stopifnot(rp1+rp2==len)
g<-get("g_density",envir=envir_stacomi)
g+
    ggplot2::scale_fill_manual(values=c(colrp(rp1),colrp1(rp2)))
...
# 1 => lancer le report interannual
...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
load(file=str_c(datawd,'bmiala2025.Rdata'))
...
bmi2023 <- summary(bmi,year_choice=2023,silent=FALSE)
bmi2024 <- summary(bmi,year_choice=2024,silent=FALSE)
bmi2025 <- summary(bmi,year_choice=2025,silent=FALSE)
datmens <- data.frame("2016"=c("capture (nb)"=round(sum(datpech2016$nb)),
    ...
    "2023"=c("capture (nb)"=0,
		"capture (kg)"=0,
		"migration(03)"=bmi2023[["5"]][1,6],
		"migration(04)"=bmi2023[["5"]][2,6],
		"migration(05)"=bmi2023[["5"]][3,6],
		"migration(06-07)"=NA),
    "2024"=c("capture (nb)"=0,
        "capture (kg)"=0,
        "migration(03)"=bmi2024[["5"]][1,6],
        "migration(04)"=bmi2024[["5"]][2,6],
        "migration(05)"=bmi2024[["5"]][3,6],
        "migration(06-07)"=NA),
    "2025"=c("capture (nb)"=0,
        "capture (kg)"=0,
        "migration(03)"=bmi2025[["5"]][1,6],
        "migration(04)"=bmi2025[["5"]][2,6],
        "migration(05)"=bmi2025[["5"]][3,6],
        "migration(06-07)"=NA)
)
...
xtala <- xtable(datmens2,
    caption=str_c("Migrations mensuelles par la passe à bassins du barrage d'Arzal entre 2016 et ", CY,
			", et captures de la pêche professionnelle en estuaire."
	),
...
dat <- data.frame("annee"=2002:2025,
    "effectif_capture"=c(177,127,60,293,360,93,134,82,549,376,2499,2994,2633,1330,271,1009,0,0,0,0,0,0,0,0),
    "poids_capture"=c(rep(NA,10),4112,4993,4105,2617,456,1707,0,0,0,0,0,0,0,0))
...
g <- ggplot(ala2) +
  geom_bar(
    aes(x = annee, y = effectif, fill = source),
    stat = "identity"
  ) +
  ylim(0, 5000) +
  scale_fill_manual(
...
plus la migration en eau douce sera longue, plus le saumon devra rester
longtemps en mer pour accumuler des réserves qui lui permettront d'atteindre les
frayères.
...
mais la situation se dégrade ces dernières années. Si historiquement la Vilaine
accueillait beaucoup de saumons, les migrations sont aujourd'hui très faibles.
L'espèce est classée vulnérable en France sur la liste rouge UICN.
}
\end{minipage}}
\smallskip

\Sexpr{vvv$SAT$N[vvv$SAT$N$annee==CY,"effectif"]} saumon a été compté en
...
La Vilaine reflète l'effondrement des retours de Saumons constaté en Bretagne et
dans la partie sud de l'aire de répartition.

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/barplot_report_annual_sat.png}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/barchart_bmi_sat.png}
...
\input{\Sexpr{vvv[["SAT"]][["path"]]}} % table bilan pour l'année

La migration des saumons était très majoritairement centrée sur le mois de juin
jusqu'au début des années 2010, puis elle s'est décalé sur juillet et août
ensuite, comme c'est le cas en \Sexpr{CY} (Figures \ref{rmi_density_sat} et
\ref{barchart_bmi_sat}).

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/rmi_density_sat.png}
...
en Figure \ref{size_sat}. On remarque que les saumons fréquentant la Vilaine
depuis 2000 sont très majoritairement des castillons.

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/size_sat.png}
...
# utilisation de la méthode plot pour générer un graphique des densités
# puis modification des couleurs du titre et de y
plot(rmi,plot.type="density",year_choice = CY, silent=TRUE)
g <- get("g_density",envir=envir_stacomi)
nb_annee<-length(unique(rmi@data$bjo_annee))
...
png(filename=str_c(imgwdy,"size_trm.png"))
ggplot(r_sample_char@data) +
    geom_boxplot(aes(x=annee,y=car_valeur_quantitatif), outlier.alpha = 0) +
    ggplot2::ylab("taille video (mm)")+
    ggplot2::xlab("annee")+
...
La truite est classée en statut quasi menacé (NT) en France sur la
...
Seule une truite de mer a été observée %ont été
en \Sexpr{CY} (Figure \ref{barplot_report_annual_trm}). %,  Tableau
%\ref{table_trm_mois}).
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/barplot_report_annual_trm.png}
...
%\begin{figure}[htpb]
%\centering
%\includegraphics[width=0.45\textwidth]{2025/barchart_bmi_trm.png}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/rmi_density_trm.png}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/size_trm.png}
...
<<report_interannual_trm, echo=FALSE, eval=FALSE, include=TRUE >>=
rmi<-new("report_mig_interannual")
rmi<-choice_c(rmi,
    dc=c(5),
    taxa=c("Salmo trutta trutta"),
    ...
    silent=TRUE)
rmi<-charge(rmi)
rmi<-connect(rmi,check=TRUE,silent=TRUE)
rmi<-calcule(rmi)
rmi@data$bjo_valeur[rmi@data$bjo_valeur<0]<- -rmi@data$bjo_valeur[rmi@data$bjo_valeur<0]
# utilisation de la méthode plot pour générer un graphique des densités
# puis modification des couleurs du titre et de y
tryCatch(plot(rmi, plot.type="density", silent=TRUE, year_choice = CY),
		error=function(e) {
			print(e)
			cat(sprintf("reverting to %s \n", tail(unique(rmi@data$bjo_annee),1)))
			plot(rmi, plot.type="density", silent=TRUE, year_choice = tail(unique(rmi@data$bjo_annee),1))
		})
...
tryCatch(
		...
		error=function(e) {
			print(e)
			cat(sprintf("reverting to %s \n", tail(unique(rmi@data$bjo_annee),1)))
			plot(rmi,plot.type="barchart",timesplit="mois", year_choice = tail(unique(rmi@data$bjo_annee),1), silent = TRUE)
		})
...
espèces recensées sont assez nombreuses : silure, sandre, black bass,
brème et cyprinidés (Figure \ref{autres_especes}).
La fréquentation des autres espèces depuis 2000 est illustrée de manière
graphique à la Figure \ref{autres_especes_2000_2025}.


Les effectifs de bars, qui utilisent la passe pour chasser et parfois migrer en
eau douce, sont en nette augmentation depuis le début de leur dénombrement
en 2022 (Figure \ref{barchart_bmi_bar}, Tableau \ref{table_bilanannuel_bar}).
L'espèce était peu présente les années précédentes, et même absente avant 2010.

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/autres_especes.png}
...
\textit{Cyprinus carpio} carpe,
\textit{Cyprinidae} cyprinidés sans détermination,
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/autres_especes_2000_2025.png}
\caption{Effectif des autres espèces rencontrées dans la passe d'Arzal depuis
2000.}
\label{autres_especes_2000_2025}
\end{figure}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/barchart_bmi_bar.png}
\caption{Effectifs mensuels de bars au droit de la vitre en \Sexpr{CY} et
comparaison aux valeurs historiques.}
\label{barchart_bmi_bar}
\end{figure}

\input{\Sexpr{vvv[["BAR"]][["path"]]}} % table bilan bars


Les effectifs de silures recensés au niveau de la passe d'Arzal sont assez
stables depuis 2018 (Figure \ref{bilan_annuel_sil}). De même, la tendance des
tailles mesurées était à l'augmentation jusqu'en 2022, puis elle semble se
stabiliser après une année 2023 marquées par un retour de plus d'individus de
petite taille (Figure \ref{size_sil}).

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/size_sil.png}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2025/bilan_annuel_sil.png}
...
ggplot(bilesp@calcdata, aes(x="",y=(Effectif / sum(Effectif))*100,fill=taxa_stage))+
	...
	guides(fill = guide_legend(title = "Taxon :", title.theme = element_text(size = 12),
					label.theme = element_text(size = 12)))
dev.off()
@

<<report_interannual_silurus, echo=FALSE, eval=FALSE,include=FALSE >>=
rmi<-new("report_mig_interannual")
rmi<-choice_c(rmi,
    dc=c(5),
...
<<report_annual_silurus, echo=FALSE, eval=FALSE,include=FALSE >>=
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Bilan annuel des migrations
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ra<-new("report_annual")
...
<<analyse_didson_silurus, echo=FALSE, eval=FALSE,include=FALSE >>=
...
dev.off()
@
<<report_interannual_dicentrarchus, echo=FALSE, eval=FALSE,include=FALSE >>=
rmi<-new("report_mig_interannual")
rmi<-choice_c(rmi,
    dc=c(5),
    taxa=c("Dicentrarchus labrax"),
    stage=c("IND"),
    start_year="1996",
    end_year=CY,
    silent=TRUE)
...
colors<-rev(mycolorramp(nb_annee))
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
png(filename=str_c(imgwdy,"rmi_density_bar.png"))
g+ggplot2::scale_fill_manual(values=colors)+
    ggplot2::ggtitle("")+
    theme(
        panel.border = element_blank(),
...
g <- g+ggtitle("")
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
png(filename=str_c(imgwdy,"barchart_bmi_bar.png"))
print(g)
dev.off()
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#png(filename=str_c(imgwdy,"plot_seasonal_trm.png"),width=14,height=10,unit="cm",res=300)
...
mmm <- mmm[,-1]
colnames(mmm) <- c("Mois", "Min*", "Moyenne*", "Max*", "N (2025)")
xtmmm<-xtable(mmm,caption="Migrations mensuelles des bars dans la passe à bassins du barrage d'Arzal. $N$ = nombre, * correspondent à
        la période 1996-2025. min* = minimum mensuel, max* = maximum mensuel, moyenne* = moyenne des effectifs mensuels.",
    label="table_trm_mois",
    digits=c(0,0,0,0,0,0),
    display = c("s","s","f","f", "f", "f"))
...
#vvv[["LPM"]]<- list()# normalement vient du rapport de l'années précédente
vvv[["BAR"]][["path_y"]]<-path_y
save(vvv,file=str_c(datawdy,"vvv.Rdata"))


@

<<report_annual_dicentrarchus, echo=FALSE, eval=FALSE, include=FALSE >>=
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Bilan annuel des migrations
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ra<-new("report_annual")
ra<-choice_c(ra,
    ...
    silent=TRUE)
ra <- connect(ra,silent=FALSE)
ra@data <- ra@data[ra@data$annee>2021,]
xtra<-stacomiR::xtable(ra,
    caption="Bars observés dans la passe à bassins du barrage
        d'Arzal (passe à fentes verticales). Les comptages ont commencé en 2022.",
    label="table_bilanannuel_bar",
    dc_name=c("P. bassins")
)
# below not run but one can create a file as following
...
print(xtra,
    ...
    add.to.row= list(pos=list(0),
        command=
            "\\multicolumn{2}{c}{\\textit{Dicentrarchus labrax}}  \\\\
            \\multicolumn{2}{c}{Indéterminé} \\\\"),
    include.rownames = TRUE,
    include.colnames = FALSE,
    format.args = list(big.mark = " ", decimal.mark = ","),
...
# some manual editing needed....
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
png(filename=str_c(imgwdy,"bilan_annuel_dicentrarchus.png"))
barplot(ra,args.legend=list(x="topleft",bty = "n"),
    col="orchid4",
    ylab="Effectif (bar)")
dev.off()
...
<<report_sample_char_dicentrarchus, echo=FALSE, eval=FALSE, include=FALSE >>=
...
png(filename=str_c(imgwdy,"size_dicentrarchus.png"))
ggplot(r_sample_char@data) +
    geom_boxplot(aes(x=mois,y=car_valeur_quantitatif), outlier.alpha = 0) +
    ggplot2::ylab("taille video (mm)")+
    ggplot2::xlab("mois")+
    geom_jitter(aes(x=mois,y=car_valeur_quantitatif, col=tax_nom_latin),
        width = 0.2,
        height = 0.2,
        alpha=0.5)+
    theme_bw()
...
Pour l'étude Glanispomi, qui vise à mesurer la prédation exercée par le
silure sur les poissons migrateurs, il est nécessaire de calculer la biomasse
sur la Vilaine pour les mois de mars à mai, pour les aloses, lamproies marines
et mulets. L'objectif est de faire ce calcul pour 2025 mais aussi pour les
années précédentes.

<<calcul_export_frederic_glanispomi, echo=FALSE, eval=FALSE,include=FALSE >>=

W = a * L^b # L en cm
# https://fishbase.se/summary/Alosa-alosa.html

vac <- c("Alosa"=0.0041,"Petromyzon marinus"=0.00208,"Chelon ramada"=0.0089)
vbc <- c("Alosa"=3.24,"Petromyzon marinus"=3,"Chelon ramada"=3.03)

weight <- function(species, L , va=vac, vb=vbc ) {
	if (any(L[!is.na(L)] >100)) warning("taille > 100 êtes vous en mm ? La formule marche avec des cm")
	a <- va[species]
	b <- vb[species]
	W <- a * L^b
}
...
# récupère les abondances des poissons de mars à mai
...
fn <- function (y) summary(rmi, y)[["5"]]
res0 <- bind_rows(mapply(fn, 2018:2025, SIMPLIFY =FALSE))
res0$tax_nom_latin = "Petromyzon marinus"
...
res2 <- bind_rows(mapply(fn, 2018:2025, SIMPLIFY =FALSE))
res2$tax_nom_latin = "Alosa"
...
res3 <- bind_rows(mapply(fn, 2018:2025, SIMPLIFY =FALSE))
res3$tax_nom_latin = "Chelon ramada"
res <- rbind(res0,res2,res3)

# récupère les structures en taille des poissons de mars à mai.
b_carlot<-new("report_sample_char")
b_carlot<-choice_c(b_carlot,
		dc=c(5),
		taxa=c("Petromyzon marinus", "Alosa","Chelon ramada"),
		stage=c("11", "11", "IND"),
		par=c("C001"),
		horodatedebut=as.Date("2018-01-01"),
		horodatefin=as.Date(str_c(CY+1,"-01-01")),
		silent=FALSE)
b_carlot <- charge(b_carlot)
b_carlot <- connect(b_carlot)
b_carlot <- calcule(b_carlot,silent=TRUE)
plot(b_carlot,plot.type='2')

tt <- b_carlot@data |> group_by(annee,mois, tax_nom_latin) |> summarize(taille_moyenne=mean(car_valeur_quantitatif), taille_sd = sd(car_valeur_quantitatif)) |> ungroup()
# convertit les tailles moyennes en poids

tt$poids_moyen <- weight(species=tt$tax_nom_latin,L=tt$taille_moyenne/10)
tt$poids_moyen_sup <- weight(species=tt$tax_nom_latin,L=tt$taille_moyenne/10 + 2*tt$taille_sd/10)
tt$poids_moyen_inf <- weight(species=tt$tax_nom_latin,L=tt$taille_moyenne/10 - 2*tt$taille_sd/10)
# calcule la biomasse des poissons
tt <- killfactor(as.data.frame(tt))



ggplot(tt) + geom_point(aes(x=mois, y= poids_moyen, col=annee)) +
		geom_path(aes(x=mois,y=poids_moyen,group=annee, col=annee)) +
		geom_errorbar(aes(x=mois,ymin=poids_moyen_sup, ymax=poids_moyen_inf,col=annee))+
		facet_wrap(~tax_nom_latin, scale="free_y") +
		viridis::scale_colour_viridis(discrete=TRUE, direction = -1) +
		theme_bw()

ggplot(tt) +
		geom_errorbar(aes(x=mois,ymin=taille_moyenne-2*taille_sd, ymax=taille_moyenne+2*taille_sd,col=annee), alpha=0.5)+
		geom_point(aes(x=mois, y= taille_moyenne, col=annee)) +
		geom_path(aes(x=mois,y=taille_moyenne,group=annee, col=annee)) +
		facet_wrap(~tax_nom_latin, scale="free_y")+
		viridis::scale_colour_viridis(discrete=TRUE, direction = -1) +
		theme_bw()
...
res <- res |> rename("annee"="année")

mois <- c("02","03","04","05","06","07","08","09","10","11","12")
names(mois) <-  c("févr.","mars","avr.","mai","juin","juil.","août","sept.","oct.","nov.","déc.")
res$mois <- unname(mois[res$mois])
restt <- inner_join(res,tt, by=c("tax_nom_latin","annee","mois"))

restt <- restt |> rename("effectif"="valeur")
restt$biomasse <- restt$effectif * restt$poids_moyen/1000
restt$biomasse_sup <- restt$effectif * (restt$poids_moyen_sup)/1000
restt$biomasse_inf <- restt$effectif * (restt$poids_moyen_inf)/1000

ggplot(restt) +
		geom_errorbar(aes(x=mois,ymin=biomasse_inf, ymax=biomasse_sup,col=annee))+
		geom_point(aes(x=mois, y= biomasse, col=annee)) +
		geom_path(aes(x=mois,y=biomasse,group=annee, col=annee)) +
		facet_wrap(~tax_nom_latin, scale="free_y")



ggplot(restt) + geom_point(aes(x=mois, y= poids_moyen, col=annee)) +
		geom_path(aes(x=mois,y=poids_moyen,group=annee, col=annee)) +
		geom_errorbar(aes(x=mois,ymin=poids_moyen_sup, ymax=poids_moyen_inf,col=annee))+
		facet_wrap(~tax_nom_latin, scale="free_y") +
		viridis::scale_colour_viridis(discrete=TRUE, direction = -1) +
...
save(restt, file= str_c(datawd, "biomasse_Vilaine.Rdata"))
load(file= str_c(datawd, "biomasse_Vilaine.Rdata"))

restt |> filter(mois %in% c("03","04","05","06")) |> group_by(annee, tax_nom_latin) |>
		summarize(effectif = sum(effectif, na.rm =TRUE),
				biomasse=sum(biomasse, na.rm =TRUE),
				biomasse_sup= sum(biomasse_sup, na.rm =TRUE),
			    biomasse_inf=sum(biomasse_inf, na.rm =TRUE))
@
...
au fonctionnement des vannes au pas de temps de 10 minutes. Le fichier des données
extrait de la base de données du barrage contient
\Sexpr{num(vvv$fonct_pass$nb_lignes)} lignes pour l'année \Sexpr{CY}. A partir
de ce dernier, un script de traitement extrait les temps et les modes de fonctionnement de la
passe (Figure \ref{figure_schema_fonctionnement}).
Les sondes utilisées en référence ont parfois des valeurs manquantes,
...
\Sexpr{num(vvv$fonct_pass$niveau_vilaine_manquant_final)} lignes pour les
...
\Sexpr{num(vvv$fonct_pass$pb_niveau_mer_inf_3.5)} lignes, celles du sas aval \footnote{Le dernier
bassin en aval de la passe. La différence de niveaux entre ce bassin et la mer
conditionne le fonctionnement de la vanne aval. Suppression des valeurs en
dehors de la gamme [-1m,2.5m].} correspondent à \Sexpr{num(vvv$fonct_pass$pb_niveau_sas)} lignes et des
...
\begin{figure}[htpb]
\centering
\includegraphics[width=\textwidth]{2025/detail_fonct_passe_2.png}
\caption{Illustration du fonctionnement de la passe lors des manip halameaup en
Avril.
A chaque cycle de marée, le niveau de la mer varie entre +2 m et -2m environ (- noir). Le niveau Vilaine (.- bleu) varie peu et reste
autour de 1m90. La vanne amont (- marron) s'ouvre dès que le niveau mer
descend 30 cm en dessous du niveau Vilaine, c'est l'entrée en régulation de la
passe. La vanne aval (... violet) ajuste le niveau du sas (- bleu)
pour maintenir une chute constante à l'aval de la passe à l'aide d'une
régulation par l'automate.
Les arrêts ponctuels pour la manip provoquent une fermeture de la vanne amont
lors des basses mer. La passe est vidée pour aller chercher les Lamproies dans
les nasses. L'état de la passe est calculé pour chaque période de 10 minutes (en bas).
Le 22 l'absence de régulation sur la vanne aval (les pointillés bleus de la
vanne aval ne descendent plus et le niveau dans le sas reste haut) conditionnent
le calcul d'arrêt pour maintenance, les fermetures de la vanne amont à basse
mer, les 23 et 24 entrainent d'autres arrêts pour maintenance.}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.8\textwidth]{2025/detail_fonct_passe_1.png}
\caption{Illustration du fonctionnement de la passe après la remise en route
de la passe en mars. En ordonnée, les niveaux mer (- noir), vilaine
(.- bleu), sas (- bleu), niveau vanne mer (... violet), niveau vanne amont
(- marron). L'état de la passe est calculée pour chaque période de 10 minutes (en bas).
La passe rouvre après une crue et des travaux le 06 mars. D'autres arrêts sont
observés le 14 mars (défauts de fin de course sur la vanne amont...)}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.6\textwidth]{2025/debit_passe_mesure.png}
\caption{Débit calculé dans la passe en 2025, mesures de l'automate du barrage.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.6\textwidth]{2025/debit_passe_calcul.png}
\caption{Débit recalculé dans la passe en 2025, après correction des valeurs
...
\begin{figure}[hp]
\centering
\includegraphics[width=0.9\textwidth]{2025/report_df_annee.png}
\caption{Bilan du fonctionnement de la passe (dispositif de franchissement) lors
de l'année 2025.
En orange \textcolor{jaune_EV}{\rule[-0.2mm]{3mm}{2mm}} il est possible de suivre les
périodes de haute mer où la passe est fermée, les
...
Vilaine est plus réduite. Les arrêts
(\textcolor{turquoise_EV}{\rule[-0.2mm]{3mm}{2mm}}) sont visibles
...
%\begin{figure}[hp]
%\centering
%\includegraphics[width=0.95\textwidth]{2025/report_df_fct_2003_2010.png}
...
\begin{figure}[hp]
\centering
\includegraphics[width=\textwidth]{2025/bmm_alose.png}
\caption{Bilan de la migration des aloses en 2025. Effectifs journaliers et
...
\begin{figure}[hp]
\centering
\includegraphics[width=\textwidth]{2025/bmm_mulet.png}
\caption{Bilan de la migration des mulets en 2025. Effectifs journaliers et
fonctionnement des dispositifs de franchissement et de comptage.}
\label{bmm_mulet}
\end{figure}
...
\begin{figure}[hp]
\centering
\includegraphics[width=\textwidth]{2025/bmm_lpm.png}
\caption{Bilan de la migration des lamproies marines en 2025. Effectifs
...
inox.\\
2011 & Modernisation des armoires électriques amont et aval,
...
leds.\\
2017-18 & Rénovation des vérins de la vanne aval.\\
2020 & Changement de pièces sur les vérins de la vanne aval.\\
...
2023 & Remplacement des néons led du caisson lumineux. Changement de la sonde
de niveau du sas. Installation d’un chauffage et d’un déshumidifacteur afin de
préserver le matériel électronique du local de la passe à bassins. Changement
des boîtiers électroniques des sondes de niveaux amont et aval du barrage.\\
2024 & Remplacement des distributeurs de la vanne amont. Remplacement des
flexibles corrodés sur les vérins de la vanne aval.\\
2025 & Remise à neuf des gabions aval et des siphons. Réparation des fins
de courses de la vanne amont.\\
\bottomrule
\end{tabular}
...
\midrule
2024 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/IQBVsT9_BeqASbWz-IilJEz3AUCBlAPxvKa5Flsk1U2XHMo?e=WSiWpH}{Suivi
de la passe à bassins, 2024}\\
2023 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/EYivEAGQThBKiYqQJJA_A6MBQnb0H1cLQzqHLQpH5bvEyw?e=Tkm3RY}{Suivi
de la passe à bassins, 2023}\\
2022 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/EUqlfhsKgqpMkzWSuEo9uRABXEw73VtgoVhM8t-C6zGTiQ?e=7w6tii}{Suivi
de la passe à bassins, 2022}\\
2021 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/ESot35-t3RFPo6Gc4X3NxkgBkvRkIochrpfMnuzpBCNZaw?e=c3Qbrv}{Suivi
de la passe à bassins, 2021}\\
2020 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/EeLSKt0Q_qxMqW6p5D4-ZIwBBBTGqgeJwmOQfIjQ3AfdWw?e=woq9fI}{Suivi
de la passe à bassins, 2020}\\
2019 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/EX0SnyoGmBlMsO-6-Il8wugBCHYxha3l2A-5tA9yTCqwbg?e=kNBHJd}{Suivi
de la passe à bassins, 2019}\\
2019 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/Eae-mGNEl0VHtdehKYfYHPcBwTXL4qV_AvuPRLJAa1eGuA?e=2cq3Bn}{Dynamique
de la population de Mulet porc \textit{Chelon ramada} (Risso, 1827) dans le
bassin versant de la Vilaine}\\
2018 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/EZddtXquW7NMjK-GwjxsDGoBD7BJw9gGTplSoalP8BHaEA?e=Tn5HXK}{Suivi
de la passe à bassins, 2018}\\
2017 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/ETdJfcpV1pZMv6BapxKjUX4BLqwMqJy7txGuPWJv2aAYOg?e=lwXGLw}{Suivi
de la passe à bassins, 2017}\\
2016 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/ES85jK5Bv0JMsTOjcf93iicB5a35QRyWhWUrSiF5qfdtog?e=ws3ARj}{Suivi
...
2010 &
\href{https://eptbvilaine56.sharepoint.com/:b:/g/extranet/ETcl4LTYbb5FqGalUwApPucBFInwPrGUdBV20pmuPfhkog?e=cfSoKj}{Suivi
de la passe à bassins, 2010}\\
2009 &
...
d'intervention des barragistes : Alban Le Guen, Clément Guillemot, Cyril Faucon,
Johann Dahlem et Laurent Philippot. Nous tenons à les remercier pour leur aide
...
\href{https://bretagne-environnement.fr/tableau-de-bord/peuplements-piscicoles-des-cours-deau-bretons}{{
https://bretagne-environnement.fr/tableau-de-bord/peuplements-piscicoles-des-cours-deau-bretons}}

Rhin Meuse Migrateurs (R2M)~:
\href{https://www.saumon-rhin.com/}{{https://www.rm-migrateurs.fr/}}
...
\textbf{Résumé}\par
 ...
 Ce rapport présente le bilan du suivi de la passe à bassins d'Arzal en
    ...
    espèces ayant transité par la passe à l'estuaire de la Vilaine.
    En \Sexpr{num(CY)},
    \Sexpr{num(vvv$ALA$NCY)} aloses (\textit{Alosa
    Sp.}),
    \Sexpr{num(vvv$LPM$NCY)} lamproies marines (\textit{Petromyzon
    marinus}),
    \Sexpr{num(vvv$TRM$NCY)} truite de mer (\textit{Salmo trutta trutta}),
    \Sexpr{num(vvv$SAT$NCY)} saumon (\textit{Salmo salar}) et
    \Sexpr{num(vvv$MUP$NCY)} mulets porcs (\textit{Chelon ramada})
   ...
   \textbf{Abstract}\par
    ...
    device, along with the annual number of the main species crossing the dam in the Vilaine estuary.
    In \Sexpr{num(CY)},
    \Sexpr{num(vvv$ALA$NCY)} shads (\textit{Alosa Sp.}),
    \Sexpr{num(vvv$LPM$NCY)} sea lampreys (\textit{Petromyzon marinus}),
    \Sexpr{num(vvv$TRM$NCY)} sea trout (\textit{Salmo trutta trutta}),
    \Sexpr{num(vvv$SAT$NCY)} salmon (\textit{Salmo salar})
    and \Sexpr{num(vvv$MUP$NCY)} mulets (\textit{Chelon ramada})
    have been counted. \par
    ...
    \vspace{8mm}
     ...
     \textit{mulet, sea lamprey, shad, stacomi}
\end{tcolorbox}
...
StacomiR 
\color{bleu_clair_EV}
\citep{briand_stacomir_2017}\par
