...
poissons migrateurs par la passe à bassins en 2025 \rev{ TODO 
...
dispositif spécifique pour les anguilles composé de deux rampes de reptation
...
Le fonctionnement de la passe en \Sexpr{CY} a été perturbé la grande crue de
janvier - février et par les travaux sur les siphons et les gabions. 
Le lancement a eu lieu le 17 février, à ce stade, les travaux des gabions ne
sont pas terminés, et les support des siphons ont été posés mais pas les coudes
finaux.
Le 19 février la passe est arrêtée en journée pour les siphons, elle sera
ensuite arrêtée du 24/02 au 07/03. En mai des problèmes techniques conduisent
au changement du fin de course haut de la vanne amont. En mai et en juin la
passe est manoeuvrée en fermeture à basse mer pour capturer les lamproies dans
le cadre de l'étude INRAe HALAMEAUP. Plusieurs problèmes techniques ont
nécessité des arrêts nocturnes sur quatre périodes entre juin et septembre, pour une durée cumulée d'un mois. Fin septembre, des fuites d'huile ont été détectées sur les flexibles des vérins de la porte aval, ce qui a
entraîné l'arrêt de la passe du 26 septembre jusqu'à leur remplacement le 19
décembre (Figure \ref{report_df_annee} en annexe).
De fin décembre à début janvier une intervention technique pour modifier une
pièce de contrôle hydraulique des vérins.
...
graphiques sont présentés pour la période 2003-\Sexpr{CY} (Figure
...
\caption{Fonctionnement de la passe de 2003 à 2025, avec les détails par type
de fonctionnement.}
...
\caption{Fonctionnement de la passe de 2003 à 2025.}
...
Le dispositif de comptage a été mis en service le 9 février et arrêté le 26
septembre. Les seuls arrêts du suivi sont dus à trois extinctions du caisson
lumineux entre le 12 et 15 février, pour une durée cumulée de 56 heures
(Figure \ref{report_dc_2025}).
...
L'année \Sexpr{CY} a été marquée par des débits de crue au premier et au
dernier trimestre, et globalement des débits bien supérieurs à ceux de
...
1997, en 2005, et de 2013 à \Sexpr{CY}. Lors des pics de migration, le comptage
...
calcul du nombre de mulets migrant jusqu'au mois de septembre (exclu) est donné au
...
période 1996-\Sexpr{CY}, elles sont même à leur minimum observé en mars (probablement du fait des arrêts de la passe) 
et mai (Figure \ref{barchart_bmi_lpm}, Tableau \ref{table_lpm_mois}).
...
d'avril à mai. Les premières migrations sont plus précoces que ce que l'on
...
depuis 2015. C’est à partir de 2019 qu’une nouvelle méthode de mesure a été 
appliquée : la taille a été mesurée relativement à 9 sections de la
vitre de comptage pour prendre en compte les déformations de la lentille de la caméra. 
Or, l’aumentation est observée depuis 2015, et ne peut donc pas être attribuée
...
Après un pic de taille dans les années 2020 et 2020, puis une baisse en 2022
et 2023 les tailles des lamproies se stabilisent.  (Figure
\ref{size_lpm}).
...
de 2019 à \Sexpr{CY} ont été calculées en prenant en compte la déformation des 
distances dans les différentes zones de la vitre.}
\label{size_lpm}
\end{figure}
...
par rapport aux années 2018-2024. En vidéo comptage, il n'est pas possible de
...
Globalement, les migrations sont inférieures à la moyenne interannuelle (Figures
...
comptage pour prendre en compte les déformations de la lentille de la caméra. Ainsi les tailles avant 2019 et 2025
...
particulièrement précoce depuis 2022 (Figure \ref{plot_seasonal_ala}).
...
\caption{Passage des aloses par pas de temps de 10 minutes en fonction de
l'heure et du niveau mer en
\Sexpr{CY}.}
...
\ref{rmi_density_sat}). En \Sexpr{CY}, la migration s'est faite
en juillet et août (Figure \ref{barchart_bmi_sat}).
...
en Figure \ref{size_sat}. On remarque que les saumons fréquentant la Vilaine sont 
très majoritairement des castillons.
...
espèces recensées sont assez nombreuses : silure, sandre, black bass, cyprinidés
et brème (Figure \ref{autres_especes}). 
...
Les effectifs de bars qui utilisent la passe pour chasser sont en très nette
augmentation  Figure \ref{barchart_bmi_bar}. 
...
nette augmentation (Figure \ref{bilan_annuel_sil}). De même, la tendance des
tailles mesurées est à l'augmentation, à l'exception des deux dernières années
pour lesquelles on observe plus de silures de petite taille (Figure \ref{size_sil}).
...
\Sexpr{num(vvv$fonct_pass$nb_lignes)} lignes \Sexpr{CY}. A partir de ce dernier,
...
2025 & Remplacement des distributeurs de la vanne amont. Remplacement des
flexibles corrodés sur les vérins de la vanne aval.\\
2025 & Réparation des fins de courses de la vanne aval.\\
\bottomrule
\end{tabular}
...
Laurent Philippot. Nous tenons à les remercier pour leur aide
