...
DBI::dbExecute(req@connection, "INSERT INTO iav.t_periodefonctdispositif_per (
        ...
        , per_tar_code
        ,'IAV' as per_org_code
        FROM t_periodefonctdispositif_per_temp") #2129
...
dbExecute(req@connection,glue("DELETE FROM  iav.tj_conditionenvironnementale_env 
            ...
            AND extract(year from env_date_debut)={CY}") ) # 346
...
DBI::dbExecute(req@connection, "INSERT INTO iav.tj_conditionenvironnementale_env (
        ...
        ,env_stm_identifiant
        ,env_org_code
        FROM tj_conditionenvironnementale_env_temp") #2012 (2021) 2309 (2022) 1982 (2024) (2025)
...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
load(file=str_c(datawd,'bmiala2023.Rdata'))
...
dat <- data.frame("annee"=2002:2025,
    "effectif_capture"=c(177,127,60,293,360,93,134,82,549,376,2499,2994,2633,1330,271,1009,0,0,0,0,0,0,0),
    "poids_capture"=c(rep(NA,10),4112,4993,4105,2617,456,1707,0,0,0,0,0,0,0))
