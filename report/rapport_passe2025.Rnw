...
Le fonctionnement de la passe en \Sexpr{CY} a été perturbé par les crues et plusieurs
avaries techniques. Le lancement a eu lieu le 2 février, après un mois de
janvier avec des forts débits. Trois crues ont ensuite perturbé le bon
fonctionnement de la passe, avec près d'un mois et demi de fermeture mi-février,
en mars et début avril. Les distributeurs de la vanne amont ont provoqué
quelques arrêts, et ont été changé rapidement début avril. Plusieurs problèmes
techniques ont nécessité des arrêts nocturnes sur quatre périodes entre juin et
septembre, pour une durée cumulée d'un mois. Fin septembre, des fuites d'huile
ont été détectées sur les flexibles des vérins de la porte aval, ce qui a
...
supporter à l’aller et au retour de migration \citep{briand_suivi_2018b}.
...
période 1996-\Sexpr{CY} (Figure \ref{barchart_bmi_lpm}, Tableau
\ref{table_lpm_mois}).
...
et 2023 les tailles des lamproies reviennent à des valeurs plus importantes,
mais elles n'ont été échantillonnées qu'en début de saison (perte d'un disque dur lors 
des mesures de taille) (Figure
\ref{size_lpm}).
...
par rapport à l'ensemble des années de migration. En vidéo comptage, il n'est
pas possible de distinguer les deux espèces, grande alose \textit{Alosa alosa} 
et alose feinte \textit{Alosa fallax}.
...
xtala <- xtable(datmens2,
    caption=str_c("Migrations mensuelles par la passe à bassins du barrage d'Arzal entre 2016 et", CY,
			", et captures professionnelles en estuaire."
	),
...
Aucune truite de mer n'a été observée %ont été
...
colors<-rev(mycolorramp(nb_annee))
#============================================
png(filename=str_c(imgwdy,"rmi_density_dicentrarchus.png"))
g+ggplot2::scale_fill_manual(values=colors)+
    ggplot2::ggtitle("")+
    theme(
        panel.border = element_blank(),
...
g <- g+ggtitle("")
#============================================
png(filename=str_c(imgwdy,"barchart_bmi_dcentrarchus.png"))
print(g)
dev.off()
#============================================
#png(filename=str_c(imgwdy,"plot_seasonal_trm.png"),width=14,height=10,unit="cm",res=300)
