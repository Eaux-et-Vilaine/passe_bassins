...
# mais ici j'ai fait des modifications manuelles en sql/traitementç2023
...
png(filename=paste0(imgwdy,"report_df_2003_2023.png"),width=6,height=12, units="in",res=300)
...
png(filename=paste0(imgwdy,"report_df_fct_2003_2023.png"),width=6,height=12, units="in",res=300)
...
#png(filename=paste0(imgwdy,"report_df_2011_2023.png"),width=8,height=12, units="in",res=300)
...
#dev.off()
#
#png(filename=paste0(imgwdy,"report_df_fct_2011_2023.png"),width=8,height=12, units="in",res=300)
...
png(filename=paste0(imgwdy,"report_dc_1998_2023.png"),width=6,height=12, units="in",res=300)
g_report_dc_1 +
...
res <- dbGetQuery(req@connection,glue("SELECT lot_identifiant, lot_commentaires 
            ...
            WHERE (ope_date_debut, ope_date_fin) overlaps ('2007-01-01'::timestamp without time zone, '2023-12-31 23:59:59'::timestamp without time zone)  
...
pred <-
    data.frame("date"=
            as.POSIXct(seq(from=as.POSIXct(dmy("01012013")), to = as.POSIXct(dmy("30072023")), by="1 day")))
pred <- fndate(pred)
...
ggplot(na.omit(data_long), aes(x = annee, y = value, fill = type)) +
...
bmi <- calcule(bmi)
#save(bmi,file=str_c(datawd,'bmimup2023.Rdata'))
#load(file=str_c(datawd,'bmi2023.Rdata'))
#str(bmi)
...
mmm <- mmm[,-1]
options(encoding = "UTF-8")
colnames(mmm) <- c("Mois", "Min*", "Moyenne*", "Max*", "N (2023)")
xtmmm <- xtable(mmm,caption="Migrations mensuelles des mulets dans la passe à bassins du barrage d'Arzal, $N$ nombre, * correspondent à
        la période 1996-2023. min* minimum mensuel, max* maximum mensuel, moyenne* moyenne des effectifs mensuels.",
...
bmi<-calcule(bmi)
save(bmi,file=str_c(datawd,'bmiala2023.Rdata'))
#load(file=str_c(datawd,'bmiala2023.Rdata'))
#str(bmi)
####################
# graphique de stacomi
##########################
plot(bmi, year_choice = CY, silent=TRUE)
g=get("g_standard",envir_stacomi)
nb_annee<-length(unique(bmi@data$bjo_annee))
...
bmi2016[["5"]][is.na(bmi2016[["5"]])]<-0
bmi2017[["5"]][is.na(bmi2017[["5"]])]<-0
bmi2018[["5"]][is.na(bmi2018[["5"]])]<-0
bmi2019[["5"]][is.na(bmi2019[["5"]])]<-0
bmi2020[["5"]][is.na(bmi2020[["5"]])]<-0
bmi2021[["5"]][is.na(bmi2021[["5"]])]<-0
bmi2022[["5"]][is.na(bmi2022[["5"]])]<-0
bmi2023[["5"]][is.na(bmi2023[["5"]])]<-0
datmens <- data.frame("2016"=c("capture (nb)"=round(sum(datpech2016$nb)),
        "capture (kg)"=round(sum(datpech2016$capture/1000)),
        "migration(03)"=0,
        "migration(04)"=bmi2016[["5"]][2,6],
        "migration(05)"=bmi2016[["5"]][3,6],
        "migration(06-07)"=round(sum(bmi2016[["5"]][c(4,5),6]))),
    "2017"=c("capture (nb)"=round(sum(datpech2017$nb)),
        ...
        "migration(06-07)"=round(sum(bmi2017[["5"]][c(4,5),6]))),
    "2018"=c("capture (nb)"=0,
        "capture (kg)"=0,
        "migration(03)"=0,
...
dat <- data.frame("annee"=2002:2023,
...
dev.off()
@
<<report_mig_mult_silurus, echo=FALSE, eval=FALSE,include=FALSE >>=
color_ope<-sample(c(mycolorramp1(10),mycolorramp2(10)),20)
bMM_Arzal <- new("report_mig_mult")
bMM_Arzal <- choice_c(bMM_Arzal,
    dc=c(5),
...
png(file=str_c(imgwdy,"bmm_silurus.png"),7,7,units="in",res=300)
plot(bMM_Arzal,plot.type="standard",
    color=color,
    color_ope=color_ope,
    main=str_c("Effectif de silures ", CY),
    cex.main=1.1,		
    type="h",
    silent=TRUE)
dev.off()
...
rmi <- choice_c(rmi,
    ...
    silent=FALSE)
rmi <- charge(rmi)
rmi <- connect(rmi, check = FALSE, silent = FALSE)	
rmi <- calcule(rmi)
...
res0 <- bind_rows(mapply(fn, 2018:2023, SIMPLIFY =FALSE))
...
res2 <- bind_rows(mapply(fn, 2018:2023, SIMPLIFY =FALSE))
...
res3 <- bind_rows(mapply(fn, 2018:2023, SIMPLIFY =FALSE))
