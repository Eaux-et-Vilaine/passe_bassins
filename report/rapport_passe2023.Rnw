...
\Sexpr{vvv$ref_dc$date_fin} (Figure \ref{report_dc_2023}).
...
\rev{La comparaison des tendances mensuelles avec les migrations
interannuelles montre qu'elles sont inférieures à la moyenne observée sur la
période 1996-2020 (Figure \ref{barchart_bmi_lpm}, Tableau
\ref{table_lpm_mois}).}
...
de mars à mai. \rev{Les premières migrations sont clairement plus précoces, et
...
va <- c("alosa_alosa"=0.0041,"petromyzon_marinus"=0.00208,"chelon_labrosus"=0.0089)
vb <- c("alosa_alosa"=3.24,"petromyzon_marinus"=3,"chelon_labrosus"=3.03)

weight <- function(species, L , va, vb ) {
	if (any(L >100)) warning("taille > 100 êtes vous en mm ? La formule marche avec des cm")
	a <- va[species]
	b <- vb[species]
	W <- a * L^b
}
