...
dans le dernier bassin à l’aval de la passe (Niveau SAS) est situé 50 cm en dessus du niveau mer, 
la passe est considérée en dysfonctionnement hydraulique.
...
Le fonctionnement de la passe en 2024 a été plutôt mauvais ...
...
1997, en 2013, 2005 et de 2013 à 2024. Lors des pics de migration, le comptage
...
<<bilan_car_lots_mup, echo=FALSE, eval=TRUE, include=FALSE >>=
...
<<modele_taille_mup, echo=FALSE, eval=TRUE, include=FALSE >>=
load_package("mgcv")
...
<<bilan_annuels_mup, echo=FALSE, eval=TRUE,include=FALSE >>=
...
<<report_mig_mult_mulet, echo=FALSE, eval=TRUE,include=FALSE >>=
...
<<report_mig_internnual_mulet, echo=FALSE, eval=TRUE,include=FALSE >>=
###########################################
...
ont été très mauvaises, mais les saisons 2020 et 2024 marque un retour à la
normale.
...
Cependant, pour l'année 2024, comme pour l'année précédente, les tailles des
...
de 2019 et 2024 ont été calculées en prenant en compte la déformation des distances dans les
différentes zones de la vitre.}
\label{size_lpm}
\end{figure}

<<report_mig_mult_lpm, echo=FALSE, eval=TRUE,include=FALSE >>=
# Lamproies --------------------------------------- 
color_ope <- sample(c(mycolorramp1(10),mycolorramp2(10)),20)
bMM_Arzal <- new("report_mig_mult")
bMM_Arzal <- choice_c(bMM_Arzal,
...
@
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
<<report_interannual_lpm, echo=FALSE, eval=TRUE, include=FALSE >>=
...
<<report_annual_lpm, echo=FALSE, eval=TRUE,include=FALSE >>=
###############################
...
<<analyse_lpm, echo=FALSE, eval=TRUE, include=FALSE >>=
load(file=str_c(datawdy,"vvv.Rdata"))
total <- vvv[["LPM"]][["total"]]
...
<<report_sample_char_lpm, echo=FALSE, eval=TRUE,include=FALSE >>=
library("stacomiR")
...
<<report_mig_mult_ala, echo=FALSE, eval=TRUE,include=FALSE >>=
...
<<struct_taille_ala, echo=FALSE, eval=TRUE,include=FALSE >>=
...
<<donnees_capture, echo=FALSE, eval=TRUE,include=FALSE >>=
...
<<report_interannual_ala, echo=FALSE, eval=TRUE,include=FALSE >>=
...
<<report_annual_ala, echo=FALSE, eval=TRUE,include=FALSE >>=
###############################
# Bilan annuel des migrations
###############################
ra	<-new("report_annual")
...
<<report_mig_mult_sat, echo=FALSE, eval=TRUE,include=FALSE >>=
color_ope<-sample(c(mycolorramp1(10),mycolorramp2(10)),20)
bMM_Arzal <- new("report_mig_mult")
bMM_Arzal <- choice_c(bMM_Arzal,
    dc=c(5),
...
<<report_interannual_sat, echo=FALSE, eval=TRUE, include=FALSE >>=
rmi <- new("report_mig_interannual")
rmi <- choice_c(rmi,
    dc=c(5),
    taxa=c("Salmo salar"),
...
<<report_annual_sat, echo=FALSE, eval=TRUE,include=FALSE >>=
###############################
# Bilan annuel des migrations
###############################
ra <- new("report_annual")
...
<<report_sample_char_sat, echo=FALSE, eval=TRUE,include=FALSE >>=
library("stacomiR")
...
<<report_interannual_trt, echo=FALSE, eval=TRUE, include=FALSE >>=
...
<<report_sample_char_trt, echo=FALSE, eval=TRUE,include=FALSE >>=
library("stacomiR")
...
comptées en \Sexpr{CY} (Figure \ref{barplot_report_annual_trm}) %,  Tableau
%\ref{table_trm_mois}).
\begin{figure}[htpb]
\centering
...
\label{size_trm}
\end{figure}
<<report_mig_mult_trm, echo=FALSE, eval=TRUE,include=FALSE >>=
color_ope<-sample(c(mycolorramp1(10),mycolorramp2(10)),20)
bMM_Arzal <- new("report_mig_mult")
bMM_Arzal <- choice_c(bMM_Arzal,
    dc=c(5),
...
<<report_interannual_trm, echo=FALSE, eval=TRUE, include=FALSE >>=
rmi<-new("report_mig_interannual")
rmi<-choice_c(rmi,
    dc=c(5),
    taxa=c("Salmo trutta trutta"),
...
<<report_annual_trm, echo=FALSE, eval=TRUE, include=FALSE >>=
###############################
# Bilan annuel des migrations
###############################
ra<-new("report_annual")
...
<<report_sample_char_trm, echo=FALSE, eval=TRUE,include=FALSE >>=
library("stacomiR")
...
tailles mesurées est à l'augmentation, même si pour l'année 2024 les silures
...
<<report_species, echo=FALSE, eval=TRUE,include=FALSE >>=
...
<<report_interannual_silurus, echo=FALSE, eval=TRUE,include=FALSE >>=
rmi<-new("report_mig_interannual")
rmi<-choice_c(rmi,
    dc=c(5),
...
<<report_annual_silurus, echo=FALSE, eval=TRUE,include=FALSE >>=
###############################
# Bilan annuel des migrations
###############################
ra<-new("report_annual")
...
<<report_sample_char_silurus, echo=FALSE, eval=TRUE,include=FALSE >>=
library("stacomiR")
...
<<analyse_didson_silurus, echo=FALSE, eval=TRUE,include=FALSE >>=
library("stacomiR")
...
dev.off()
@
<<report_interannual_dicentrarchus, echo=FALSE, eval=TRUE,include=FALSE >>=
rmi<-new("report_mig_interannual")
rmi<-choice_c(rmi,
    dc=c(5),
...
<<report_sample_char_dicentrarchus, echo=FALSE, eval=TRUE,include=FALSE >>=
library("stacomiR")
...
mulets. L'objectif est de faire ce calcul pour 2024 mais aussi pour les années
précédentes.
...
contient \Sexpr{num(vvv$fonct_pass$nb_lignes)} lignes. A partir de ce dernier,
...
2024 & TODOTODO. Remplacement des flexibles corrodés de la vanne aval.\\
\bottomrule
\end{tabular}
...
\textbf{Résumé}\par
 ...
 Ce rapport présente le bilan de suivi de la passe à bassins d'Arzal en
   ...
   \textbf{Abstract}\par
    ...
    and \Sexpr{num(vvv$MUP$NCY)} mulets (\textit{Chelon ramada}) have been
