...
poissons migrateurs par la passe à bassins en 2024 \rev{ TODO 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2024/bmm_horaires_maree} 
...
mig <-	stacomiR::fun_date_extraction(
    data=mig,
...
load(file=file.path(datawd, CY, "fonct_pass.Rdata"))	# ll
mig$time <- force_tz(strptime(strftime(mig$ope_date_debut, format="%Y-%m-%d %H:%M"), "%Y-%m-%d %H:%M"), tz = "Europe/Paris")
mig$timec <- force_tz(as.POSIXct(strptime(strftime(mig$ope_date_debut, format="%H:%M:%S"), format="%H:%M:%S")), "Europe/Paris") # no date
...
png(file=str_c(imgwdy,"bmm_horaires_mareebis.png"),7,7,units="in",res=300)
ggplot(mige)+
...
dat <- data.frame("annee"=2002:2024,
    "effectif_capture"=c(177,127,60,293,360,93,134,82,549,376,2499,2994,2633,1330,271,1009,0,0,0,0,0,0),
    "poids_capture"=c(rep(NA,10),4112,4993,4105,2617,456,1707,0,0,0,0,0,0))
