...
head(res1$time)
head(lubridate::with_tz(res1$time, tzone = "Europe/Paris"))
attributes(res1$time)$tzone <- "Europe/Paris"
res1 <- res1 %>%
    mutate("time"=time-as.difftime(1, units = "hours"))		


res1 %>% filter(Horodate >'2024-21-05 00:00:00')
...
DBI::dbExecute(req@connection, "INSERT INTO iav.tj_conditionenvironnementale_env (
        ...
        ,env_stm_identifiant
        ,env_org_code
        FROM tj_conditionenvironnementale_env_temp") #2012 (2021) 2309 (2022)
...
TRES PEU DE MESURES,voir si dd 2.5p gris est retrouvé
Contrairement aux années 2022 et 2023, les tailles des lamproies en 2024 sont en
baisse par rapport aux années 2019, 2020 et 2021 (Figure \ref{size_lpm}).
...
augmentation avec des tailles allant en s'accroissant, devra se poser 
l'année prochaine si les effectifs n'augmentent pas.
...
\caption{Bilan du fonctionnement de la passe lors de l'année 2024. En orange
\textcolor{jaune_EV}{\rule[-0.2mm]{3mm}{2mm}} il est possible de suivre les 
...
Vilaine est plus réduite. Les arrêts nocturnes sont visibles de juin à
septembre (\textcolor{jaune_EV}{\rule[-0.2mm]{3mm}{2mm}}).
En hiver et au printemps, en période de crue, la passe est à l'arrêt.}
