...
%%   2- dans pdata/traitement_stacomi/rapport_passeY créer des sous répertoires
...
%%   4 - Lancer les chunks, si des données externes sont à charger voir avec cédric, lancer avec knitr.
%%   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
du barrage d'Arzal, Rapport 2021)}, 
...
\fancyhead[LO,RE]{\slshape \normalsize Rapport passe à bassins 2021}
...
\graphicspath{{../../../pdata/traitement_stacomi/rapport_passe/image/}}
...
setwd("C:/workspace/p/traitement_stacomi/rapport_passe")
datawd<-"C:/workspace/pdata/traitement_stacomi/rapport_passe/data/"
load_package <- function(x)
{
	if (!is.character(x)) stop("Package should be a string")
	if (!require(x,character.only = TRUE))
...
imgwd<-"C:/workspace/pdata/traitement_stacomi/rapport_passe/image/"
# le répertoire contenant les tableaux en latex
tabwd<-"C:/workspace/pdata/traitement_stacomi/rapport_passe/table/"
...
poissons migrateurs par la passe à bassins en 2021 \footnote{la migration sur
...
Le fonctionnement de la passe en 2021 a été plutôt bon. Il y a eu des arrêts en
...
intervention sur les capteurs de la vanne aval. En 2021, les débits sont restés soutenus et la passe n'est pas passée en
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/fonctionnement_passe.png}
...
En figure \ref{report_df_fct_2003_2021}, l'analyse des durées mensuelles
...
\ref{report_df_2003_2021}) en mettant juste en avant les périodes où la passe est fonctionnelle
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/debit_passe_comp.png}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/report_df_fct_2003_2021.png}
\caption{Fonctionnement de la passe de 2003 à 2021, avec les détails par type
de fonctionnement.}
\label{report_df_fct_2003_2021}
\end{figure}%
%
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/report_df_2003_2021.png}
\caption{Fonctionnement de la passe de 2003 à 2021.}
\label{report_df_2003_2021}
\end{figure}%
%
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/report_dc_2021.png} 
\caption{Bilan du fonctionnement du dispositif de comptage en \Sexpr{CY}.}
\label{report_dc_2021}
\end{figure}

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/report_dc_1998_2021.png} 
\caption{Bilan du fonctionnement du dispositif de comptage de 1998 à 2021.}
\label{report_dc_1998_2021}
\end{figure}
...
ll <- calc_fonct_pass(dat = data_DF_init)
data_DF <- ll$dat
...
graph_DF_detail(data_DF, from="08/04/2021")
dev.off()

png(filename=file.path(imgwd,CY,"detail_fonct_passe_2.png"),7,7,units="in",res=300)
graph_DF_detail(data_DF=data_DF, from="15-05-2021", to="19-05-2021")
dev.off()
...
#jour de l'annee
debit_jour_recalcul<-tapply(data_DF$debit_passe_recalcule*600,list(as.factor(data_DF$jour)),sum,na.rm = TRUE)
debit_jour_automate<-tapply(data_DF$debit_passe*600,list(as.factor(data_DF$jour)),sum,na.rm = TRUE)
...
table_jour$debit_jour_recalcul[index_jour_recalcul] <- debit_jour_recalcul
table_jour$debit_jour_automate[index_jour_automate] <- debit_jour_automate
...
# mais ici j'ai fait des modifications manuelles en sql/traitementç2021
...
png(filename=paste0(imgwdy,"report_df_2003_2021.png"),width=6,height=12, units="in",res=300)
...
png(filename=paste0(imgwdy,"report_df_fct_2003_2021.png"),width=6,height=12, units="in",res=300)
...
#png(filename=paste0(imgwdy,"report_df_2011_2021.png"),width=8,height=12, units="in",res=300)
...
#dev.off()
#
#png(filename=paste0(imgwdy,"report_df_fct_2011_2021.png"),width=8,height=12, units="in",res=300)
...
png(filename=paste0(imgwdy,"report_dc_1998_2021.png"),width=6,height=12, units="in",res=300)
g_report_dc_1 +
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/fig_debit.png} 
...
1997, en 2013 et de 2015 à 2021. Lors des pics de migration, le comptage des
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/graphe_taille_mulets.pdf} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/bilan_annuel_mup} 
\caption{Bilan annuel des migrations de mulets au barrage d'Arzal.}
\label{bilan_annuel_mup}
\end{figure}

Les effectifs de mulets de 2021
N=\Sexpr{format(vvv[["MUP"]][["NCY"]], digits=1, nsmall =0, big.mark=" ",scientific=FALSE)}
sont comparables à ceux de la période 2014-2021 avec des variations entre des effectifs de 200~000 à 600~000 mulets. 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/horaires_mulet} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/bmm_horaires_maree} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.9\linewidth,keepaspectratio]{2021/rmi_journalier_mup.png} 
...
pred <-
		data.frame("date"=
						as.POSIXct(seq(from=as.POSIXct(dmy("01012013")), to = as.POSIXct(dmy("30072021")), by="1 day")))
pred <- fndate(pred)
...
bmi <- calcule(bmi)
#save(bmi,file=str_c(datawd,'bmimup2021.Rdata'))
#load(file=str_c(datawd,'bmi2021.Rdata'))
#str(bmi)
...
mmm <- mmm[,-1]
options(encoding = "UTF-8")
colnames(mmm) <- c("Mois", "Min*", "Moyenne*", "Max*", "N (2021)")
...
ont été très mauvaises, mais les saisons 2020 et 2021 marque un retour à la
normale.
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/barplot_report_annual_lpm.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/barchart_bmi_lpm.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/rmi_density_lpm.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.49\textwidth]{2021/plot_seasonal_lpm.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/size_lpm.png} 
\caption{Comparaison des structure en taille des lamproies marines. Les tailles
de 2019 et 2021 ont été calculées en prenant en compte la déformation des distances dans les
...
%\begin{figure}[htpb]
%\centering
%\includegraphics[width=0.45\textwidth]{2021/bilan_annuel_ala.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/rmi_journalier_ala.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/barchart_bmi_ala.png} 
\caption{Comparaison de la migration d'aloses de \Sexpr{CY} avec la série
interannuelle de données 1996-\Sexpr{CY-1}.}
...
déformations de la lentille de la caméra. Ainsi les tailles avant 2019 et 2021
...
aloses.). Cette même cohorte se retrouve en 2021. Celà signifierait un succès
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/size_alose.png}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.49\textwidth]{2021/plot_seasonal_ala.png} 
...
\begin{figure}[pbt]
\centering
\includegraphics[width=0.5\textwidth]{2021/tx_exploit.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.9\linewidth]{2021/preco_diagram.png} 
...
bmi<-connect(bmi,silent=FALSE)
bmi<-calcule(bmi)
save(bmi,file=str_c(datawd,'bmiala2021.Rdata'))
#load(file=str_c(datawd,'bmi2021.Rdata'))
str(bmi)
####################
...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
load(file=str_c(datawd,'bmiala2021.Rdata'))
...
dat <- data.frame("annee"=2002:2021,
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/barplot_report_annual_sat.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/barchart_bmi_sat.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/rmi_density_sat.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/size_sat.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/barplot_report_annual_trm.png} 
...
%\begin{figure}[htpb]
%\centering
%\includegraphics[width=0.45\textwidth]{2021/barchart_bmi_trm.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/rmi_density_trm.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/size_trm.png} 
...
graphique à la Figure \ref{autres_especes_2000_2021}. 

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/autres_especes.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/autres_especes_2000_2021.png} 
\caption{Effectif des autres espèces rencontrées dans la passe d'Arzal depuis
2000.}
\label{autres_especes_2000_2021}
\end{figure}


\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/bilan_annuel_sil.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2021/size_sil.png} 
...
\begin{figure}[htpb]
\centering
\includegraphics[width=\textwidth]{2021/detail_fonct_passe_2.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.6\textwidth]{2021/debit_passe_mesure.png}
\caption{Débit calculé dans la passe en 2021, mesures de l'automate du barrage.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.6\textwidth]{2021/debit_passe_calcul.png}
\caption{Débit recalculé dans la passe en 2021, après correction des valeurs
...
\begin{figure}[hp]
\centering
\includegraphics[width=0.9\textwidth]{2021/report_df_annee.png} 
\caption{Bilan du fonctionnement de la passe lors de l'année 2021. En orange
...
\begin{figure}[hp]
\centering
\includegraphics[width=0.95\textwidth]{2021/report_df_fct_2003_2010.png} 
...
\begin{figure}[hp]
\centering
\includegraphics[width=\textwidth]{2021/bmm_alose.png}
\caption{Bilan de la migration des aloses en 2021. Effectifs journaliers et
...
\begin{figure}[hp]
\centering
\includegraphics[width=\textwidth]{2021/bmm_mulet.png}
\caption{Bilan de la migration des mulets en 2021. Effectifs journaliers et
fonctionnement des dispositifs de franchissement et de comptage.}
\label{bmm_mulet}
\end{figure}
...
\begin{figure}[hp]
\centering
\includegraphics[width=\textwidth]{2021/bmm_lpm.png}
\caption{Bilan de la migration des lamproies marines en 2021. Effectifs
