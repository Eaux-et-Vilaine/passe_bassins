...
fin d'année et des débits printaniers plus faibles que les débits de
référence (modules mensuels) de la banque hydro (Figure \ref{fig_debit}).
\begin{figure}[htpb]
\centering
...
tailles moyennes des mulets reconstituées en calculat la taille moyenne pondérée
à partir du modèle de variation saisonnière de la taille et des effectifs.
...
sont pas connues, mais une tendance exactement similaire a été rapportée
sur la Loire (Marion Legrand, comm. pers.)(Figure
\ref{size_lpm}, \ref{size_lpm_density}).
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.8\textwidth]{2021/detail_fonct_passe_1.png}
