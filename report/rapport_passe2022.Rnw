...
la passe (fermer la vanne amont) en marée decendante, et privilégier son
fonctionnement lors de la marée montante. Le canal de débit d'appoint, chenal
qui amènent un débit supplémentaire de 1m3/S à l'aval de la passe est également
battardé. Ce fonctionnement augmente les périodes arrêt pour maintenance qui
...
\rev[red]{Les interventions techniques sur les vannes sont assez limitées,
\begin{itemize}
...
\rev{L'année \Sexpr{CY} a été marquée par des débits de crue en début et en
...
tailles moyennes des mulets reconstituées en calculant la taille moyenne
...
sont à leur minimum historique sur l'ensemble des saisons de suivi.
...
gris les minimum, maximum et moyenne des valeurs historiques de migration
mensuelles.}
...
###################################
load(file=str_c(dat_char_2183awdy,"r_sample_char_mulet.Rdat_char_2183a"))
...
g5 <- g1+
 ...
 ggrepel::geom_text_repel(aes(x=year,y=taille_ponderee,label=label_effectif), col=orange_EVf,  
     nudge_y=0.9,
...
\rev{La migration des lamproies marines intervient entre mars et juin (Figure
\ref{rmi_density_lpm}). En \Sexpr{CY}, la migration s'est faite essentiellement
de février à mai, ce qui contraste avec l'année précédente où les migration
...
\rev{La comparaison des structures en taille vidéo laisse paraître un
accroissement depuis 2015. Les raisons de cette augmentation de taille ne sont pas connues,
mais une tendance exactement similaire avait été rappor‐
tée sur la Loire (Marion Legrand, comm. pers.). Cependant, pour l'année 2022 les
...
\rev{Globalement, les migrations sont inférieures à la
...
\rev{Une truite de mer a été %ont été
...
Vilaine est plus réduite. Entre juillet et octobre le passage en "mode étiage" réduit la
