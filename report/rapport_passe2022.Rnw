...
Le fonctionnement de la passe en 2022 a été plutôt bon. La passe n'est pas
fonctionnelle en débit de crue car la baisse de la ligne d'eau amont limite
fortement les débits évacués et la passe est très peu attractive, mais ce
dysfonctionnement n'apparait pas sur les chroniques des figures
\ref{figure_fonctionnement_passe} \ref{report_df_fct_2003_2022} et \ref{report_df_2003_2022}.
Le 28/03, il y a eu une intervention technique sur les centrales hydrauliques pour changer l'huile et
le fonctionnement des vannes est re-réglé. 
Le 27/04 la passe est arrêtée pour installer des cordes à l'aval pour tenter de
gêner les braconniers. Le 26/08 il y a un problème d'automate.
La passe est passée en mode étiage (diminution du débit,
et battardage du débit d'appoint) de juillet à novembre.
...
Le suivi est fonctionnel sauf deux arrêts en septembre et octobre
equi demandent un changement de disque (Figure \ref{report_dc_2022}).
...
la tendance saisonnière des migrations.
...
g3 <- g2 +  
    ...
    geom_point(aes(x=ope_date_,debut,y=pred, fill=annee), 
...
Typically, the workflow of a data-poor stock
assessment with LFQ data would include: (i) estimation of
biological stock characteristics (growth and natural mortality),
(ii) fisheries performance aspects (exploitation rate and
selectivity), and (iii) stock size and status. The order of the
methods is important as they build upon each other in a
sequential way. If some or all of the vital parameters for
stock assessment are already known, the user may skip the
data-poor approaches for their assessment and can directly
TropFishR \citep{mildenberger_tropfishr_2017}
...
Ms <- M_empirical(Linf = lfq_bin2$par$Linf, 
    K_l = lfq_bin2$par$K, 
    method = "Then_growth")
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2022/size_lpm_density.png} 
\caption{Comparaison des structure en taille des lamproies marines.}
\label{size_lpm_density}
\end{figure}
