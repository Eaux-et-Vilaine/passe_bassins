...
Le fonctionnement de la passe en 2022 a été plutôt bon. En été la passe est
passée en mode étiage de juillet à novembre. Le mode étiage consiste à arrêter
la passe (fermer la vanne amont) en marée descendante, et privilégier son
fonctionnement lors de la marée montante. Le canal de débit d'appoint, chenal
qui amène un débit supplémentaire de 1m3/S à l'aval de la passe est également
batardé. Ce fonctionnement augmente les périodes arrêt pour maintenance qui
interviennent lorsque la passe devrait en théorie être fonctionnelle 
(les niveaux permettent son ouverture) alors qu'en pratique elle est fermée (Figures \ref{figure_fonctionnement_passe} \ref{report_df_fct_2003_2022} et \ref{report_df_2003_2022}).
Les interventions techniques sur les vannes sont assez limitées,
\begin{itemize}
\item le 28/03, il y a eu une intervention technique sur les centrales
hydrauliques pour changer l'huile et le fonctionnement des vannes a fait l'objet d'un règlage. 
\item le 27/04 la passe a été arrêtée pour installer des cordes à l'aval pour
tenter de gêner les braconniers. Le 26/08 il y a un problème d'automate.
\end{itemize}
...
\paragraph{Calculs du fonctionnement et des débits de la passe à bassins.}
...
ponctuels (en jaune \textcolor{jaune_EV}{\rule[-0.2mm]{3mm}{2mm}}). Les mêmes
...
\Sexpr{vvv$ref_dc$date_debut} après que les premiers mulets aient été observés
dans la passe. %\Sexpr{vvv$ref_dc$date_fin}
Le suivi est fonctionnel sauf lors de deux arrêts en septembre et octobre,
problèmes techniques au niveau du matériel informatique qui ont demandé un
changement de disque (Figure \ref{report_dc_2022}).
Sur l'ensemble de la période de suivi depuis 2003, les arrêts techniques pendant
la période de suivi sont relativement rares (Figure \ref{report_dc_1998_2022}). 
La caméra a été recalée en début de saison et fait l'objet d'une recalibration.
Les néons de la passe ont été remplacés en 2022 par des néons à leds. La vitre
est très rayée ce qui entraine un problème de salissure, et de qualité de
l'image.
...
fin d'année et sur le reste de l'année des débits plus
faibles que ceux de référence (modules mensuels) de la banque hydro (Figure
\ref{fig_debit}).
\begin{figure}[htpb]
\centering
...
Cette année des mulets de petite taille ont été observés en mai, et on retrouve
la tendance saisonnière des migrations. Il n'y a pas de tendance claire des
tailles moyennes reconstituées en calculant la taille moyenne
pondérée à partir du modèle de variation saisonnière de la taille et des effectifs.
...
automne. B. Graphique en violon et box-plot des tailles annuelles, les points
représentent la moyenne pondéree des tailles modélisées (courbe en C) par les effectifs. En
texte, effectif de mulet (N) et effectif mesuré (e).
C. Modèle gam ajusté sur les valeurs de taille (au centre modèle et intervalle de confiance
...
(N=\Sexpr{format(vvv[["MUP"]][["NCY"]], digits=1, nsmall =0, big.mark="",scientific=FALSE)}) sont à leur minimum historique sur l'ensemble des saisons de suivi.
La diminution d'un facteur 2 est alarmante. Les effectifs mensuels sont tous
soit inférieurs à la moyenne (en orange sur la Figure \ref{barchart_bmi_mup})
soit à leur minimum historique (en marron sur la Figure \ref{barchart_bmi_mup}).

\begin{figure}[htpb]
\centering
\includegraphics[width=0.45\textwidth]{2022/barchart_bmi_mup} 
\caption{Bilan inter-annuel des migrations de mulets au barrage d'Arzal. En
gris les minimum, maximum et moyenne des valeurs historiques des migrations
mensuelles.}
\label{barchart_bmi_mup}
\end{figure}
...
dat_char_2183 <- r_sample_char@data
#dat_char_2183[dat_char_2183$car_valeur_quantitatif> 700,]
#str(dat_char_2183)
dat_char_2183$date <- dat_char_2183$ope_date_debut # je cree une variable date qui sera plus tard transformée
...
dat_char_2183 <- fndate(dat_char_2183)
dat_char_2183 <- rename(dat_char_2183, taille_mm=car_valeur_quantitatif)
...
###################################
dat_char_2183$mois <- as.factor(dat_char_2183$month)
g0 <- ggplot(dat_char_2183)+
...
g1 <- ggviolin(dat_char_2183, x = "year", y = "taille_mm", fill = "year",
        palette = colours,
...
# dat_char_2183[1:100,c("time","yday","year")]


#
#g0 <- gam(taille_mm ~ s(yday),  data=dat_char_2183)
#
#
#g0m <- bam(taille_mm ~ s(yday,bs="cc")+ s(date,bs="re"),  data=dat_char_2183)
#
#g1m <- bam(taille_mm ~ s(yday,bs="cc",by=year)+ s(date,bs="re"),  data=dat_char_2183)
#
##x11()
...
#
#g3m <- bam(taille_mm ~ s(yday,bs="cc",by=year)+s(date,bs="re"),  data=dat_char_2183)
#summary(g3m)
#visreg(g3m, xvar="date",  ylab= "taille mm")
#visreg(g3m, xvar="yday", by="year", ylab= "taille mm")
g4m <- gam(taille_mm ~ s(yday,bs="cc", by=year,k=8)+s(time,k=6),  data=dat_char_2183)


plot(acf(resid(g4m)))
plot(pacf(resid(g4m))) # pas bon, il faudrait utiliser un gamm, je vais pas donner de stats
g2mm <- gam(taille_mm ~ s(time,k=3),  data=dat_char_2183)
g4mm <- gam(taille_mm ~ s(yday,bs="cc", by=year,k=8)+s(time,k=3),  data=dat_char_2183)
g3mm <- gam(taille_mm ~ s(yday,bs="cc", by=year,k=8),  data=dat_char_2183)
g5mm <- gam(taille_mm ~ s(yday,bs="cc", k=8)+s(time,k=3),  data=dat_char_2183)
...
dat_char_2183$pred <-predict(g4m, type="response",se.fit=TRUE)$fit
# 95 % des données de prédiction sont entre fit *se.fit et fit-2*se.fit
dat_char_2183$lwr <-dat_char_2183$pred-2*predict(g4m, type="response",se.fit=TRUE)$se.fit
dat_char_2183$upr <-dat_char_2183$pred+2*predict(g4m, type="response",se.fit=TRUE)$se.fit
dat_char_2183$lwrPoint <-dat_char_2183$pred - 2*allsd
dat_char_2183$uprPoint <-dat_char_2183$pred + 2*allsd
dat_char_2183$upr <-dat_char_2183$pred+2*predict(g4m, type="response",se.fit=TRUE)$se.fit
...
save(g0,g1,g2,g3, file=str_c(datawdy, "plot_taille_mulet.Rdata"))
...
pred$fit[pred$fit >600] <-0
range(pred$date)




the_seq <- seq(200,800,by=5)
...
# On vire les predictions qui partent en sucette (2013)

load_package("plot3D")
plot3D::image2D(ma, x=pred$date, y =the_seq, contour=TRUE,
    ylab="Taille (mm)")

save(ma,file=paste0(datawdy,"ma.Rdata"))
mad <- as.data.frame(ma)


plot3D::persp3D(x=, y=the_seq, z= ma)
pred2 <- cbind(pred,mad)
save(dat_char_2183,file=paste0(datawdy,"dat_char_2183.Rdata"))
save(pred2,file=paste0(datawdy,"pred2.Rdata"))
save(vvv , file=file.path(datawdy,"vvv.Rdata"))
@
...
bmi <- calcule(bmi)
save(bmi,file=str_c(datawdy,'bmimup2022.Rdata'))
...
plot(bmi,plot.type="barchart", timesplit="mois", year_choice = CY, silent=TRUE, alpha=1)
...
<<traitements_mulets, echo=FALSE, eval=FALSE,include=FALSE >>=

load(file=str_c(datawdy,'bmimup2022.Rdata')) # bmi
load(file=paste0(datawdy,"pred2.Rdata"))
load(file=paste0(datawdy,"ma.Rdata"))
load(file=str_c(datawd,"lm_taille_poids.Rdata")) 
load(file=paste0(datawdy,"dat_char_2183.Rdata"))
load( file=str_c(datawdy, "plot_taille_mulet.Rdata")) # g0, g1, g2
# on extrait que les lignes avec effectif total
effectif <- bmi@data%>%
    filter(bjo_labelquantite=="Effectif_total")
#with(effectif, plot(bjo_jour,bjo_valeur))
effectif$date <- as.Date(effectif$bjo_jour) 
# les effectifs sont bien les bons
sel <- effectif$bjo_annee>=2013
with(subset(effectif, sel),tapply(bjo_valeur,bjo_annee,sum,na.rm=TRUE))
with(subset(effectif, sel),tapply(bjo_valeur,month(date),sum))
# on joint les données journaières d'effectif au tableau de prédiction
pred3 <- left_join(effectif[,c("date","bjo_valeur")], pred2,by="date") %>% filter(!is.na(year))
pred3 <- rename(pred3,effectif=bjo_valeur)



###################################
#calcul de la taille moyenne par an
###################################
tendance_taille <- pred3 %>% mutate(taillexeffectif=fit*abs(effectif)) %>% group_by(year)%>%summarize(taille_ponderee=sum(taillexeffectif)/sum(effectif),
    effectif=sum(effectif))
n_measured <- dat_char_2183 %>% group_by(annee) %>% tally() %>% rename(year=annee)
tendance_taille <- inner_join(tendance_taille,n_measured) %>%
    mutate(label_effectif=paste0("N=",effectif,"\n e=",n))
mean_taille_ponderee <- mean(tendance_taille$taille_ponderee)

g5 <- g1+
 geom_point(aes(x=year,y=taille_ponderee), col=orange_EVf, data=tendance_taille) +
 ggrepel::geom_text_repel(aes(x=year,y=taille_ponderee,label=label_effectif), col="black", size=3, 
     nudge_y=0.9,
     data=tendance_taille
    ) +
 geom_hline(yintercept=mean_taille_ponderee, lty=3)
...
p1 <- ggarrange(g0, g5, labels=c("A", "B"),
    ncol = 2, nrow = 1)
p2 <- ggarrange(g3, labels="C")
p3 <- ggarrange(p1, p2, nrow=2, ncol=1)
...
#taille_ponderee$year <- as.numeric(as.character(taille_ponderee$year))
#plot(taille_ponderee$year,taille_ponderee$taille_ponderee,type="l",ylim=c(200,600))
...
#L <- seq(200,800,by=5)
## prediciton du poids
#df_pred_poids <- data.frame(L=L,lL=log(L/10))
#df_pred_poids$poids <- exp(predict(lm_taille_poids,newdata=df_pred_poids))
#map<- matrix(rep(df_pred_poids$poids,nrow(pred3)),
#    nrow=nrow(pred3),
#    ncol=ncol(ma),byrow=TRUE)
#
#
## effectifs par date (lignes) et classe de taille (colonnes)
#pred3[,10:ncol(pred3)] <-pred3[,10:ncol(pred3)]*matrix(rep(abs(pred3$effectif),121),
#    nrow=nrow(pred3),
#    ncol=ncol(ma), 
#    byrow=FALSE)
#ma2 <-as.matrix(pred3[,10:ncol(pred3)])
#plot3D::image2D(ma2, x=pred3$date, y =the_seq, ,
#    ylab="Taille (mm)")
#mat4 <- t(L)%*%as.matrix(t(pred3[,10:ncol(pred3)]))
#plot(t(mat4/sum(L)))
...
ont été très mauvaises. Après un retour à la normale en 2020, les migrations
sont à nouveau en forte baisse depuis 2021.
...
La première partie de l'année 2022 se caractérise par des tendances mensuelles
supérieures à la moyenne observée sur la période 1996-\Sexpr{CY}. Au contraire,
pour la deuxième partie de l'année, on observe des tendances mensuelles
inférieures à la moyenne des migrations interannuelles (Figure \ref{barchart_bmi_lpm}, Tableau
\ref{table_lpm_mois}).
...
de février à mai, ce qui contraste avec l'année précédente où les migrations
ont commencé en mars.
Les premières migrations sont clairement plus précoces, et les migrations de
février correspondent au maximum connu jusqu'alors (Figures
\ref{barchart_bmi_lpm}, \ref{plot_seasonal_lpm}).
Ces résultats sont en accord avec l'examen des tendances de migration des
lamproies marines en France par \citet{legrand_DiadromousFishModified_2021} qui
montrent globalement une arrivée plus précoce dans les 30 dernières années. 
...
paraître un accroissement entre 2015 et 2021. C’est à partir de 2019 qu’une
nouvelle méthode de mesure a été appliquée : la taille a été mesurée relati‐
...
similaire avait été rapportée sur la Loire (Marion Legrand, comm. pers.). Cependant, pour l'année 2022 les
tailles des lamproies sont en baisse par rapport aux années 2019, 2020 et 2021
(Figure \ref{size_lpm}).
...
de 2019 à 2022 ont été calculées en prenant en compte la déformation des
...
r_sample_char<-calcule(r_sample_char,silent=TRUE)
r_sample_char@data$annee <- as.factor(as.character(r_sample_char@data$annee))
r_sample_char@data <- r_sample_char@data[
    as.numeric(as.character(r_sample_char@data$annee))>2005 &
        as.numeric(as.character(r_sample_char@data$annee))!=2008,] 

# A "violin" plot
plot(r_sample_char,plot.type="1",silent=TRUE)

g<-get("g1",envir=envir_stacomi)
png(filename=str_c(imgwdy,"size_lpm.png"),6,6,units="in",res=300)
...
taille <- left_join(r_sample_char@data,
    r_sample_char@data %>% 
        group_by(annee) %>% 
        tally(),
    by= "annee")
taille$fannee <- factor(as.character(taille$annee), 
    levels=unique(taille$annee),
    labels=unique(paste0(taille$annee," (",taille$n,")")))
png(filename=str_c(imgwdy,"size_lpm_density.png"),6,6,units="in",res=300)
taille %>%
    filter(as.numeric(as.character(annee))>2005) %>%
    filter(as.numeric(as.character(annee))!=2008) %>%
    ggplot(aes(x=car_valeur_quantitatif)) +
    geom_density(aes(fill=fannee), alpha=0.5) + 
    xlim(0,1500) +
    theme_bw()
dev.off()
...
moyenne interannuelle sauf pour le mois de mai (Figures
\ref{rmi_journalier_ala}, \ref{barchart_bmi_ala}, Tableau
\ref{table_ala_mois}).
...
Ainsi, les tailles avant 2019 et 2022 sont peut être légèrement plus petites (Figure \ref{size_alose}).
...
surexploitation en 2012-2014. En 2021, les tailles ont ré-augmenté, puis elles
retournent à leur valeur de 2020 en 2022. Ces variations de taille moyennes
...
g<-get("g_density",envir=envir_stacomi)	
g+
    ggplot2::scale_fill_manual(values=c(colrp(15),colrp1(15)))
...
dat <- data.frame("annee"=2002:2022,
    "effectif_capture"=c(177,127,60,293,360,93,134,82,549,376,2499,2994,2633,1330,271,1009,0,0,0,0,0),
    "poids_capture"=c(rep(NA,10),4112,4993,4105,2617,456,1707,0,0,0,0,0))
...
rmi <- connect(rmi,check=TRUE,silent=FALSE)	
rmi <- calcule(rmi)
save(rmi,file=str_c(datawd,'rmisalmo2022.Rdata'))
#load(file=str_c(datawd,'rmisalmo2022.Rdata'))
# utilisation de la méthode plot pour générer un graphique des densités
# puis modification des couleurs du titre et de y
...
Une truite de mer a été %ont été
...
rmi<-connect(rmi,check=TRUE,silent=FALSE)	
rmi<-calcule(rmi)
save(rmi,file=str_c(datawd,'rmitrm2022.Rdata'))
#load(file=str_c(datawd,'rmitrm2022.Rdata'))
...
nombreuses, silure, sandre, bar commun, truite fario, tanche, cyprinidés et
brème (Figure \ref{autres_especes}). 
...
\textit{Dichentrachus labrax} bar commun,
\textit{Salmo trutta fario} truite fario,
\textit{Tinca tinca} tanche,
...
sont en très nette augmentation (Figures \ref{size_sil}
et \ref{bilan_annuel_sil}) .
...
autour de 2m. A partir de 16 h 00, la vanne aval (.... violet) entre en
régulation et se ferme progressivement pour maintenir une hauteur de chute
constante entre le niveau du sas (courbe bleue) et le niveau mer (courbe noire). 
Lorsqu'une différence de moins de 30 cm apparaît entre le niveau Vilaine et la
mer, la vanne amont se ferme (trait marron).
 On constate qu'il y a des fuites sur la vanne aval à pleine mer, car le niveau
 du sas monte au-dessus de 1m80.  Un arrêt pour maintenance est calculé au
 moment de la réouverture de la vanne car la passe ne s'ouvre pas immédiatement
 à l'atteinte du seuil de 30 cm. L'état de la passe est calculée pour chaque
 période de 10 minutes (en bas).
}
...
Vilaine est plus réduite. Entre juillet et octobre, le passage en "mode étiage"
réduit la durée de fonctionnement de la passe aux quelques heures qui précèdent la marée
haute. En décembre, en période de crue, la passe est à l'arrêt.}
...
\textbf{Résumé}\par
 ...
 Ce rapport présente le bilan de suivi de la passe à bassins d'Arzal en
   ...
   \textbf{Abstract}\par
    ...
    and \Sexpr{num(vvv$MUP$NCY)} mullets (\textit{Chelon ramada}) have been
